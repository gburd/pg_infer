...
fn predict_hybrid_metal(
    ...
    let dummy = infer_compute::QuantWeight {
        data: &[], scales: None, format: infer_compute::QuantFormat::Q4_0,
    };
    let attn_layers: Vec<infer_compute::FullPipelineLayer> = layer_range.clone()
        .map(|layer| {
    ...
    let metal: &infer_compute::metal::MetalBackend = unsafe {
        &*(backend as *const dyn ComputeBackend as *const infer_compute::metal::MetalBackend)
    };
