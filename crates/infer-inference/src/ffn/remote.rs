//! RemoteWalkBackend — FFN backend that dispatches to a `infer-server` over
...
//! `crates/infer-server/src/routes/walk_ffn.rs` for the endpoint.
//!
...
//! (`Content-Type: application/x-infer-ffn`), which eliminates JSON float
...
const BINARY_CT: &str = "application/x-infer-ffn";
...
impl RemoteWalkBackend {
    ...
    /// the same shard). For cross-shard batches, route through `infer-router`
    /// using JSON.
    pub fn call_batch(
        &self,
        layers: &[usize],
