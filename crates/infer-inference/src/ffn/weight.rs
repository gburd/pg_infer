...
pub fn dense_ffn_forward(
    ...
    let activation = if arch.ffn_type() == infer_models::FfnType::Gated {
        let w_gate = weights
            .tensors
        ...
        match arch.activation() {
            infer_models::Activation::GeluTanh => gelu_tanh_gate_up(&gate, &up),
        ...
        match arch.activation() {
            infer_models::Activation::GeluTanh | infer_models::Activation::Gelu => projected.mapv(gelu_tanh),
