...
/// Write a UTF-8 string into WASM linear memory via `infer_alloc`, return (ptr, len).
pub(crate) fn write_str(
    ...
    let alloc: TypedFunc<u32, u32> = instance.get_typed_func(&mut *store, "infer_alloc")?;
...
/// Call `infer_dealloc(ptr, len)` inside the module. Errors are swallowed: a
/// failed free should not mask a successful result.
fn dealloc(store: &mut Store<ExpertStore>, instance: &Instance, ptr: u32, len: u32) {
    if let Ok(f) = instance.get_typed_func::<(u32, u32), ()>(&mut *store, "infer_dealloc") {
        let _ = f.call(&mut *store, (ptr, len));
    }
}

/// Call `infer_call(op, args)` and return the parsed `ExpertResult`, or `None`
/// if the expert declined to handle the op.
///
/// Every buffer allocated inside the module's linear memory during this call
/// (op string, args string, result string) is freed via `infer_dealloc` before
...
pub fn call(
    ...
    let handle: TypedFunc<(u32, u32, u32, u32), u32> =
        instance.get_typed_func(&mut *store, "infer_call")?;
...
/// Call `infer_metadata` and return the parsed `ExpertMetadata`.
pub fn metadata(
    store: &mut Store<ExpertStore>,
    instance: &Instance,
) -> anyhow::Result<ExpertMetadata> {
    let meta_fn: TypedFunc<(), u32> = instance.get_typed_func(&mut *store, "infer_metadata")?;
