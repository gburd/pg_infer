...
mod tests {
    ...
    fn registry_or_skip() -> Option<ExpertRegistry> {
        let dir = wasm_dir();
        if !dir.exists() {
            eprintln!("skip: wasm dir missing at {} — run `cargo build --target wasm32-wasip1 --release` in infer-experts", dir.display());
            return None;
        }
...
/// even on machines that haven't built the infer-experts WASM modules.
#[cfg(test)]
mod mock_tests {
    use super::*;
