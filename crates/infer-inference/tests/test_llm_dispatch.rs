...
///
/// Requires:
///   - INFER_MODEL env var pointing to a model path or HuggingFace ID
///     (defaults to "google/gemma-3-4b-it")
///   - infer-experts pre-built for wasm32-wasip1
///
...
use infer_inference::experts::{parse_op_call, ExpertRegistry};
use serde_json::{json, Value};

// ── Infrastructure ────────────────────────────────────────────────────────────

fn model_id() -> String {
    std::env::var("INFER_MODEL").unwrap_or_else(|_| "google/gemma-3-4b-it".to_string())
}

fn wasm_dir() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../infer-experts/target/wasm32-wasip1/release")
}
...
fn llm_dispatch_pipeline() {
    // ── Pre-conditions ──
    if !wasm_dir().exists() {
        eprintln!("skip: wasm dir missing — run `cargo build --target wasm32-wasip1 --release` in infer-experts");
        return;
    }
