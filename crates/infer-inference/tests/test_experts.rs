...
///
/// Requires the infer-experts workspace to be pre-built:
///   cd crates/infer-experts && cargo build --target wasm32-wasip1 --release
///
...
use infer_inference::experts::ExpertRegistry;
...
fn wasm(name: &str) -> PathBuf {
    wasm_dir().join(format!("infer_expert_{}.wasm", name))
}
...
fn registry_memory_stable_across_many_calls() {
    // Without the infer_dealloc pairing in caller.rs, arithmetic's linear
