...
use infer_inference::{encode_prompt, forward::forward_to_layer, InferenceModel, WeightFfn};
use infer_inference::forward::generate_cached_constrained;

#[test]
fn bench_probe_vs_generate() {
    let mid = std::env::var("INFER_MODEL").unwrap_or_else(|_| "google/gemma-3-4b-it".to_string());
