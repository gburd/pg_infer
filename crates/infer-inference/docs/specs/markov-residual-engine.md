...
**Audience:** infer contributors.
**Scope:** Contract for a KV-cache-free decode engine in `infer-inference`,
...
into `infer-inference` as a first-class engine.
...
1. Lift `kv-cache-benchmark::real_model::markov_layer` into
   `infer-inference::engines::markov_residual` (or wherever the engine
   module convention lands).
2. Keep the `KvStrategy` trait impl in `kv-cache-benchmark`, but have it
   wrap `infer-inference::MarkovResidualEngine` rather than own the
...
3. The `#[ignore]`'d real-model test suite moves with the implementation;
   ...
   `KvStrategy` adapter.
4. Drop the "Tier 1 / variant iv-dense" naming. In `infer-inference`,
...
5. Document §4 preconditions against each model architecture supported
   by `infer-inference` as part of the migration PR. New architectures
...
- **Gemma 4 E4B support.** The latest measured run covers Gemma 3 4B.
  Gemma 4 E2B has been validated end-to-end in the infer research stack
...
`infer-inference::MarkovResidualEngine` via the `KvStrategy` adapter,"
