...
This crate runs transformer forward passes with Apple Accelerate (AMX) and optional Metal GPU acceleration. It uses `infer-vindex` for gate KNN (sparse feature selection) and `infer-models` for weight loading and architecture definitions.

```rust
use infer_inference::InferenceModel;

// Load a model
let model = InferenceModel::load("google/gemma-3-4b-it")?;

// Run inference
let result = infer_inference::predict(
...
| `model.rs` | Model loading (re-exports from infer-models) |

## Compute Backend

All GPU pipeline operations use `infer_compute::ComputeBackend`:

```rust
use infer_compute::{default_backend, ComputeBackend};
...
cargo build --release -p infer-inference --features metal
```
...
```bash
cargo run --release -p infer-vindex --example convert_gates_f32 -- path/to/vindex
cargo run --release -p infer-vindex --example build_down_features -- path/to/vindex
cargo run --release -p infer-vindex --example build_up_features -- path/to/vindex
```
...
```bash
cargo run --release -p infer-server -- path/to/vindex --port 8080
...
cargo run --release -p infer-inference --example bench_walk_inference -- \
...
cargo test -p infer-inference

# HNSW tests
cargo test -p infer-vindex --test test_hnsw --release

# Individual test suites
cargo test -p infer-inference --test test_fused_attention   # 23 tests
cargo test -p infer-inference --test test_backend           # 13 tests
cargo test -p infer-inference --test test_modules           # 15 tests
cargo test -p infer-inference --test test_trace             # 14 tests
cargo test -p infer-inference --test test_walkers           # 12 tests
cargo test -p infer-inference --test test_walker_utils      # 10 tests
```
...
```
infer-models      ModelWeights, architecture traits, quant
infer-compute     ComputeBackend, Q4 matvec, Metal GPU (used by vindex + walk_ffn Q4 paths)
infer-vindex      VectorIndex, gate KNN, adaptive residency, Q4 gates
infer-core        Graph, Edge, algorithms (knowledge graph engine)
    ↓
infer-inference   Forward pass, attention, backends, WalkFfn
```

> **Note:** The GPU pipeline paths (`predict_honest`, `predict_pipeline`) use `infer_compute::ComputeBackend`
> with `FullPipelineLayer` structs that carry per-layer architecture params from `infer_models::ModelArchitecture`.
