...
//!
//! Usage:
//!   cargo run --release -p infer-inference --example backend_demo
...
use infer_compute::{default_backend, ComputeBackend, MatMulOp};
...
fn main() {
    ...
    #[cfg(feature = "metal")]
    {
        ...
        if let Some(metal) = infer_compute::MetalBackend::new() {
            metal.calibrate();
