...
//!
//! Usage (two shards):
//!   infer-server <vindex> --experts 0-63  --port 9191 &
//!   infer-server <vindex> --experts 64-127 --port 9192 &
//!
...
use infer_vindex::{load_vindex_tokenizer, VectorIndex, SilentLoadCallbacks};
...
fn main() -> Result<(), BoxErr> {
    ...
    let weights = infer_vindex::load_model_weights_q4k(&vindex_path, &mut cb)?;
    ...
    #[cfg(feature = "metal")]
    let backend = infer_inference::MetalBackend::new()
        .ok_or("Metal not available")?;
    #[cfg(not(feature = "metal"))]
    let backend = infer_inference::CpuBackend;
