...
//!
//! Usage:
//!   cargo run --release -p infer-inference --example profile_overhead

use std::time::Instant;
use infer_inference::{predict, InferenceModel, WeightFfn, FfnBackend};
use infer_inference::forward::{dot_proj, apply_norm, forward_to_layer};
use ndarray::Array2;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let t0 = Instant::now();
    for _ in 0..10 {
        let _ = infer_inference::attention::run_attention_block(weights, &h, 0, false);
    }
    ...
    let t0 = Instant::now();
    for _ in 0..10 {
        let (h_post_attn, _, _) = infer_inference::attention::run_attention_block(weights, &h, 0, false).unwrap();
