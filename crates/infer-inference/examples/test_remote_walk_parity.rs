...
//! `RemoteWalkBackend` (HTTP → infer-server running on localhost) and diffs
...
//! cargo run --release -p infer-cli -- serve path/to/gemma3-4b.vindex \
//!   --port 8080 --log-level warn
//! ```
//!
//! ```bash
//! # Terminal B — run the parity check.
//! cargo run --release -p infer-inference --example remote_walk_parity -- \
...
use infer_vindex::{load_vindex_embeddings, SilentLoadCallbacks, VectorIndex};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let weights: ModelWeights = infer_vindex::load_model_weights(&vindex_path, &mut cb)?;
