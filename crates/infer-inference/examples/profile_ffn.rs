...
//!   cargo run --release -p infer-inference --example ffn_profile -- \
...
use infer_inference::{default_backend, InferenceModel};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let backend_ref: Option<&dyn infer_compute::ComputeBackend> = Some(&*backend);
