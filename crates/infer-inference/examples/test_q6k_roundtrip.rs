...
fn main() {
    ...
    let q6k = infer_compute::cpu::ops::q4_common::quantize_q6_k(&data);
    ...
    // Dequantize via ggml
    let deq = infer_models::quant::ggml::dequantize(&q6k, infer_models::quant::ggml::TYPE_Q6_K, 256);
    match deq {
        Ok(ref d) => {
    ...
    let deq2 = infer_models::quant::ggml::dequantize_q6_k(&q6k, 256);
    match deq2 {
        Ok(ref d) => {
            let nz = d.iter().filter(|v| v.abs() > 1e-6).count();
