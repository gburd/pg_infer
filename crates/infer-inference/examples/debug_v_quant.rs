...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let q6k = infer_compute::cpu::ops::q4_common::quantize_q6_k(&padded);
    println!("\nQ6_K quantized: {} bytes", q6k.len());

    // Check the d scale of first superblock
    let d_bytes = &q6k[208..210];
    let d = infer_models::quant::half::f16_to_f32(u16::from_le_bytes([d_bytes[0], d_bytes[1]]));
    ...
    // Dequantize
    let deq = infer_models::quant::ggml::dequantize_q6_k(&q6k, padded_len)?;
    ...
    let q4k_d = infer_models::quant::half::f16_to_f32(u16::from_le_bytes([q4k_d_bytes[0], q4k_d_bytes[1]]));
