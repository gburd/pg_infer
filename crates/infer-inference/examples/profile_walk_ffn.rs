...
//!
//! Run:
//!   cargo run --release -p infer-inference --example profile_walk_ffn -- \
...
fn main() {
    ...
    let gate_index: &dyn infer_vindex::GateIndex = &index;
    let hidden = weights.hidden_size;
    let intermediate = gate_index.num_features(14);
    let arch = &*weights.arch;
    let use_gelu = matches!(arch.activation(), infer_models::Activation::GeluTanh | infer_models::Activation::Gelu);
    let is_gated = arch.ffn_type() == infer_models::FfnType::Gated;
    ...
    for layer in 0..14 {
        ...
        let (h_out, _) = infer_inference::forward::run_ffn(weights, &h_post_attn, layer, &dense_ffn, false);
        h = h_out;
    }
    // Use last position
    ...
    let x_normed = infer_inference::forward::apply_norm(
    ...
    let backend = infer_inference::default_backend();
    ...
    println!("=== Step-by-step breakdown (L14, K=200, f32 gate) ===\n");
    {
        ...
        for _ in 0..iters {
            for (i, &(_, gate_score)) in hits.iter().enumerate().take(k) {
                let activated_gate = if use_gelu {
                    infer_inference::ffn::gelu_tanh(gate_score)
                } else {
                    gate_score * infer_inference::ffn::sigmoid(gate_score)
                };
        ...
        let t = Instant::now();
        for _ in 0..iters {
            let _ = infer_inference::forward::apply_norm(
        ...
        for _ in 0..iters {
            ...
            if let Some(ref dv) = down_view {
                for &(feat, gate_score) in hits.iter().take(k) {
                    let act = if use_gelu { infer_inference::ffn::gelu_tanh(gate_score) } else { gate_score * infer_inference::ffn::sigmoid(gate_score) };
                    if act.abs() > 1e-10 {
        ...
        if let Some(dv) = gate_index.down_layer_matrix(layer) {
            for _ in 0..iters {
                ...
                for &(feat, gs) in hits.iter().take(k) {
                    let act = if use_gelu { infer_inference::ffn::gelu_tanh(gs) } else { gs * infer_inference::ffn::sigmoid(gs) };
