...
//!
//! Usage:
//!   cargo run --release -p infer-inference --example profile_ffn_compute -- \
//!     --vindex output/gemma3-4b-v2.vindex

use std::time::Instant;
use ndarray::Array2;
use infer_inference::InferenceModel;
use infer_inference::forward::forward_to_layer;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut index = infer_vindex::VectorIndex::load_vindex(&vindex_path, &mut cb)?;
    ...
    let h_norm = infer_inference::forward::apply_norm(
    ...
    let _ = infer_inference::ffn::silu_gate_up(&gate_scores, &up_scores);
    let t0 = Instant::now();
    for _ in 0..n { let _ = infer_inference::ffn::silu_gate_up(&gate_scores, &up_scores); }
    let geglu_ms = t0.elapsed().as_secs_f64() * 1000.0 / n as f64;
    let activation = infer_inference::ffn::silu_gate_up(&gate_scores, &up_scores);
    ...
    let walk_ffn = infer_inference::vindex::WalkFfn::new(weights, &index, 8092);
    use infer_inference::ffn::FfnBackend;
