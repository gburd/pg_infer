...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let model = infer_inference::InferenceModel::load("google/gemma-3-4b-it")?;
    let weights = model.weights();
    let vd = std::path::PathBuf::from("output/gemma3-4b-v2.vindex");
    let mut index = infer_vindex::VectorIndex::load_vindex(&vd, &mut infer_vindex::SilentLoadCallbacks)?;
    ...
    let ffn_format = infer_compute::QuantFormat::Q4_K;

    let encoding = model.tokenizer().encode("The capital of France is", true).unwrap();
    let ids: Vec<u32> = encoding.get_ids().to_vec();
    let h = infer_inference::forward::embed_tokens_pub(weights, &ids);
    ...
    for n_layers in [1, 2, 5, 10, 20, 34] {
        let layers = infer_inference::layer_graph::pipeline_layer::build_pipeline_layers(
