...
//!
//! Run:
//!   cargo run --release --features metal -p infer-inference --example bench_hybrid -- \
...
fn main() {
    ...
    let model = infer_inference::InferenceModel::load(model_path)
    ...
    let mut index = infer_vindex::VectorIndex::load_vindex(
        &vindex_dir,
        &mut infer_vindex::SilentLoadCallbacks,
    ...
    let cached = infer_inference::CachedLayerGraph::from_residuals(vec![]);
    ...
    {
        // Warm up
        let _ = infer_inference::predict_hybrid(
        ...
        for _ in 0..iters {
            ...
            result = Some(infer_inference::predict_hybrid(
                weights, model.tokenizer(), &token_ids, 5,
                &index, &*backend, &cached, layer_range.clone(),
            ));
        }
    ...
    {
        let _ = infer_inference::predict_honest(
            weights, model.tokenizer(), &token_ids, 5,
            &index, &*backend, &cached, layer_range.clone(),
        );
        ...
        for _ in 0..iters {
            backend.reset_kv_cache();
            result = Some(infer_inference::predict_honest(
                weights, model.tokenizer(), &token_ids, 5,
                &index, &*backend, &cached, layer_range.clone(),
            ));
        }
    ...
    {
        let walk_ffn = infer_inference::vindex::WalkFfn::new(weights, &index, 8192);
