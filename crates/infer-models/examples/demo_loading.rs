...
//!
//! Shows how infer-models loads weights, detects architecture, and exposes
//! tensor information. Requires a model path as argument.
//!
//! Run: cargo run -p infer-models --example demo_loading -- /path/to/model
...
fn main() {
    ...
    if args.len() < 2 {
        ...
        println!();
        println!("Examples:");
        println!("  cargo run -p infer-models --example demo_loading -- /path/to/gemma-3-4b");
        println!("  cargo run -p infer-models --example demo_loading -- model.gguf");
        return;
    }

    let model_path = &args[1];
    println!("=== infer-models: Weight Loading Demo ===\n");

    // Resolve path (handles HF cache lookup)
    let resolved = match infer_models::resolve_model_path(model_path) {
        Ok(p) => p,
    ...
    let weights = match infer_models::load_model_dir(&resolved) {
        Ok(w) => w,
        Err(e) => {
