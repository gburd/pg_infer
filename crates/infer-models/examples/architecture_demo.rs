...
//!
//! Run: cargo run -p infer-models --example architecture_demo

use infer_models::{detect_from_json, ModelArchitecture};

fn main() {
    println!("=== infer-models: Architecture Detection Demo ===\n");
    ...
    let f16_data = infer_models::quant::half::encode_f16(&[1.0, -2.0, 2.71]);
    let f16_back = infer_models::quant::half::decode_f16(&f16_data);
    ...
    for exp in [0u8, 126, 127, 128, 130] {
        print!("{}→{} ", exp, infer_models::quant::mxfp4::e8m0_to_f32(exp));
    }
    println!();
    ...
    // ═══════════════════════════════════════════════════════════
    println!("\n=== Vector Components ===\n");
    for comp in infer_models::ALL_COMPONENTS {
        println!("  {comp}");
    }
