...
Model architecture definitions for infer — traits, config parsing, tensor key mappings, weight loading, and quantization formats.

## What it does

Describes *what a model is* without performing any computation. Every model architecture implements the `ModelArchitecture` trait, which tells the rest of infer how to find tensors, apply norms, scale embeddings, handle RoPE, and route MoE experts — all without importing a single math crate.
...
```rust
use infer_models::{detect_architecture, detect_from_json, ModelArchitecture};
...
```rust
use infer_models::load_model_dir;
...
These handle data format encoding/decoding only. Compute operations (GPU matvec, shader dispatch) are in `infer-compute`.
...
```rust
use infer_models::{VectorRecord, ALL_COMPONENTS};
...
cargo run -p infer-models --example architecture_demo

# Load a real model and inspect its structure
cargo run -p infer-models --example demo_loading -- /path/to/model

# Compare tensor key patterns across architectures
cargo run -p infer-models --example demo_tensor_keys
```
