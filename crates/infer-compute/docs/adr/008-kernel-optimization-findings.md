...
|---|----------|--------|--------|---------|
| 1 | Fused Q4_K QKV | 1.78x vs Q8 | infer original | Fusing Q+K+V eliminates dispatch overhead |
| 2 | Half-precision inner loop | No improvement | infer experiment | GPU is not ALU-throughput-bound at this size |
| 3 | Integer Q8 inner loop | No improvement | Inspired by Q4_0 v4 | Q8 quantize overhead = integer multiply savings |
| 4 | Pre-baked half scales (Q4_KF) | No improvement | infer experiment | Scale decode is <10% of per-sub-block ALU |
| 5 | Sub-block lane assignment | +3% | infer original | 80 subs / 32 lanes = better utilization |
| 6 | simd_shuffle input broadcast | Battery only | Inspired by llama.cpp | Plugged in: parallel lanes > register broadcast |
| 7 | Struct-aligned reads | Marginal | infer experiment | Compiler already coalesces byte reads |
| 8 | Pre-loaded 128B register array | Slower | infer experiment | 32 × uint32 causes register spilling |
