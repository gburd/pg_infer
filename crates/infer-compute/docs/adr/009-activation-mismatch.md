...
The CPU path in `infer-inference/src/forward.rs` correctly dispatches based on `arch.activation()`. The GPU path in `infer-compute/src/metal/decode.rs` and `full_pipeline.rs` always uses `geglu_silu`.
