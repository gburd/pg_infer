...
impl MetalBackend {
    ...
    pub fn decode_token_with_moe_fn(
        ...
        // `INFER_DUMP_RESIDUALS=<path>` is set.
        ...
        let diag_stop_layer: Option<usize> = std::env::var("INFER_DECODE_DIAG_LAYER")
        ...
        for l in 0..num_layers {
            ...
            let dump_l0_dir = if l == 0 { std::env::var("INFER_DUMP_L0").ok() } else { None };
            ...
            if has_moe {
                if let Some(ref moe) = layer.moe {
                    ...
                    // L0-only intermediate dumps for HF diff. `INFER_DUMP_L0=<dir>`
