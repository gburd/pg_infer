...
//! - `INFER_DUMP_RESIDUALS=<path>` — binary dump of every layer's
...
/// ```text
///   b"INFER_RES_V2\0\0\0\0"   (16 bytes — magic + reserved)
/// ```
...
impl ResidualDump {
    /// Create a dump sink if `INFER_DUMP_RESIDUALS` is set, otherwise a no-op.
    pub(super) fn from_env() -> Self {
        let path = match std::env::var("INFER_DUMP_RESIDUALS") {
        ...
        if let Err(e) = f.write_all(b"INFER_RES_V2\0\0\0\0") {
