...
//! Run with:
//!   cargo run --release -p infer-compute --example bench_shaders                  # CPU only
//!   cargo run --release -p infer-compute --features metal --example bench_shaders # CPU + Metal

extern crate blas_src;

use std::time::Instant;
use infer_compute::cpu::q4;
...
fn main() {
    ...
    let cpu = infer_compute::cpu_backend();
    ...
    println!("\n--- Q4 matvec (CPU C kernel) ---");
    {
        ...
        t.run("CPU C kernel [10240,2560] × x[2560]", || {
            let _ = infer_compute::cpu::ops::q4_matvec::dispatch(&q4_data, &x, inter, hidden);
        });
    }
    ...
    println!("\n--- Q4 vecmat (CPU C kernel) ---");
    {
        ...
        t.run("CPU C kernel act[10240] × Q4[10240,2560]", || {
            let _ = infer_compute::cpu::ops::q4_vecmat::dispatch(&act, &q4_data, inter, hidden);
        });
    }
    ...
    println!("\n--- GEGLU (CPU) ---");
    {
        ...
        t.run("CPU geglu silu (10240 elements)", || {
            let _ = infer_compute::cpu::ops::geglu::geglu_silu_alloc(&gate, &up);
        });
    }
    ...
    println!("\n--- Causal attention (CPU) ---");
    {
        ...
        t.run("CPU causal attention (seq=6, dim=320)", || {
            let _ = infer_compute::cpu::ops::attention::causal_attention(&q, &k, &v, seq, dim, 1.0 / (dim as f32).sqrt());
        });
        let q1 = vec![0.01f32; dim];
        let k1 = vec![0.01f32; dim];
        let v1 = vec![0.01f32; dim];
        t.run("CPU causal attention (seq=1, dim=320)", || {
            let _ = infer_compute::cpu::ops::attention::causal_attention(&q1, &k1, &v1, 1, dim, 1.0 / (dim as f32).sqrt());
        });
    }
    ...
    #[cfg(feature = "metal")]
    {
        ...
        let metal = match infer_compute::metal::MetalBackend::new() {
            Some(m) => m,
