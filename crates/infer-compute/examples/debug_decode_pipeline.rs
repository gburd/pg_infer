...
//! Runs inside infer-compute where we have direct Metal access.
//!
//! cargo run --release --features metal -p infer-compute --example debug_decode_pipeline
...
#[cfg(feature = "metal")]
fn main() {
    let metal = infer_compute::metal::MetalBackend::new().expect("need metal");
    let bufs = metal.bufs();
    ...
    let dummy_gate_q4 = infer_compute::cpu::ops::q4_common::quantize_q4_0(&gate_f32);
    let dummy_up_q4 = infer_compute::cpu::ops::q4_common::quantize_q4_0(&gate_f32);
    let down_f32: Vec<f32> = (0..hidden * inter).map(|i| ((i as f32) * 0.000002).cos() * 0.1).collect();
    let dummy_down_q4 = infer_compute::cpu::ops::q4_common::quantize_q4_0(&down_f32);

    // Build Q4_K weights for attention (synthetic)
    let wq_data = infer_compute::cpu::ops::q4_common::quantize_q4_k(
        &(0..q_dim * hidden).map(|i| ((i as f32) * 0.00001).sin() * 0.5).collect::<Vec<_>>()
    );
    let wk_data = infer_compute::cpu::ops::q4_common::quantize_q4_k(
        &(0..kv_dim * hidden).map(|i| ((i as f32) * 0.00002).cos() * 0.5).collect::<Vec<_>>()
    );
    let wv_data = infer_compute::cpu::ops::q4_common::quantize_q4_k(
        &(0..kv_dim * hidden).map(|i| ((i as f32) * 0.00003).sin() * 0.5).collect::<Vec<_>>()
    );
    let wo_data = infer_compute::cpu::ops::q4_common::quantize_q4_k(
        &(0..hidden * q_dim).map(|i| ((i as f32) * 0.00004).cos() * 0.5).collect::<Vec<_>>()
    );

    use infer_compute::{QuantWeight, QuantFormat, FullPipelineLayer, NormType, FfnType, Activation};
    ...
    let result = infer_compute::metal::MetalBackend::decode_token(
    ...
    println!("\n--- Test 2: Standalone rms_norm (offset=1.0) ---");
    {
        ...
        let result = infer_compute::metal::buffers::read_buffer_f32(&norm_out, hidden);
        let nz = result.iter().filter(|v| v.abs() > 1e-10).count();
        let max = result.iter().fold(0.0f32, |a, &b| a.max(b.abs()));
    ...
    println!("\n--- Test 3: residual_norm_q8 (offset=1.0) ---");
    {
        ...
        let f32_result = infer_compute::metal::buffers::read_buffer_f32(&f32_out, hidden);
        let nz = f32_result.iter().filter(|v| v.abs() > 1e-10).count();
        println!("  f32_out (residual): nonzero={}/{}", nz, hidden);

        let q8_data = infer_compute::metal::buffers::read_buffer_f32(&q8_scales, hidden / 32);
    ...
    println!("\n--- Test 4: decode_token with norm_offset=1.0 ---");
    {
        ...
        let r = infer_compute::metal::MetalBackend::decode_token(
