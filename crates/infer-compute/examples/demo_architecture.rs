//! Architecture Overview — guided tour of infer-compute's major design decisions.
//!
...
//!
//! Usage: cargo run --release --features metal -p infer-compute --example demo_architecture

extern crate blas_src;

fn main() {
    use infer_compute::{default_backend, cpu_backend};
    use infer_compute::cpu::ops::q4_common::{quantize_q4_0, quantize_q4_k, quantize_to_q8};
    use ndarray::Array2;
    use std::time::Instant;

    println!("╔══════════════════════════════════════════════╗");
    println!("║   infer-compute Architecture Overview        ║");
    println!("╚══════════════════════════════════════════════╝\n");
    ...
    if backend.has_q4() {
        ...
        let layer = infer_compute::FullPipelineLayer {
            wq: infer_compute::QuantWeight { data: &wq, scales: None, format: infer_compute::QuantFormat::Q4_K },
            wk: infer_compute::QuantWeight { data: &wk, scales: None, format: infer_compute::QuantFormat::Q4_K },
            wv: infer_compute::QuantWeight { data: &wv, scales: None, format: infer_compute::QuantFormat::Q4_K },
            wo: infer_compute::QuantWeight { data: &wo, scales: None, format: infer_compute::QuantFormat::Q4_K },
            gate: infer_compute::QuantWeight { data: &gate, scales: None, format: infer_compute::QuantFormat::Q4_0 },
            up: infer_compute::QuantWeight { data: &up, scales: None, format: infer_compute::QuantFormat::Q4_0 },
            down: infer_compute::QuantWeight { data: &down, scales: None, format: infer_compute::QuantFormat::Q4_0 },
