...
**Scope:** `infer-vindex`, `infer-lql`, `infer-inference`, `infer-python`
...
- **arch-B override check** (`infer_lql::executor::query::infer`): explicit cosine match against `patched.knn_store.query_top1(layer, residual)` at `cos > 0.75`, result presented as KNN override in INFER output. Runs in parallel with the dense FFN pass.
...
**Regression suite:**
- Existing 309 tests in `infer-lql` and infer-vindex must pass after the refactor (allowing for output format string updates).
...
- `infer-lql/src/executor/mutation.rs` `exec_insert` (will be rewritten)
- `infer-lql/src/executor/query.rs` `exec_infer` (KNN override branch deleted)
