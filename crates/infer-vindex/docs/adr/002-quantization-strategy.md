...
filenames. The CLI surfaces this as `infer extract-index --quant q4k`,
...
Our format separates scales and mins for simpler code. GGUF packs both into 12 bytes for 4 fewer bytes per block. Both produce equivalent results. `quantize_q4_k_gguf()` in infer-compute can produce the GGUF format.
