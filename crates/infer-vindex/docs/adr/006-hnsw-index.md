...
- Random projection: dim → 64 via `matmul` (infer-compute BLAS)
...
HNSW algorithm by Malkov & Yashunin (2016). Implementation is original infer — built on top of infer-compute's BLAS matmul for the projection step.
