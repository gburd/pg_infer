...
impl VectorIndex {
    ...
    pub fn synthesize_lm_head_q4(&mut self) {
        ...
        for row in 0..vocab {
            ...
            for (i, slot) in row_f32.iter_mut().enumerate().take(hidden) {
                ...
                *slot = infer_models::quant::half::f16_to_f32(bits);
            }
            let q4 = infer_compute::cpu::q4::quantize_q4_0(&row_f32);
    ...
    pub fn lm_head_knn(&self, query: &ndarray::Array1<f32>, top_k: usize) -> Vec<(u32, f32)> {
        ...
        // gemv via infer-compute: scores = query @ lm_head^T → [1, vocab]
        let hidden = self.hidden_size;
...
mod tests {
    ...
    fn synthesize_lm_head_q4_produces_correct_bytes() {
        ...
        for row in 0..vocab {
            ...
            let bits = infer_models::quant::half::f32_to_f16(val);
            for col in 0..hidden {
