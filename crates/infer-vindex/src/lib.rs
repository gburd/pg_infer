...
//!
//! All matrix operations route through `infer_compute` (BLAS on CPU,
//! Metal GPU when `--features metal`).

// BLAS provided by infer-compute dependency (no direct blas_src needed)
...
pub use extract::{
    ...
    build_vindex_streaming, resolve_gate_dtype,
...
// Model loading: use infer_models::{load_model_dir, resolve_model_path, load_gguf} directly
