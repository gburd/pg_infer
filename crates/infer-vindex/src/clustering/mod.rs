...
pub fn classify_direction(
    ...
    let d_norm = infer_compute::norm(&direction.view());
    if d_norm < 1e-8 {
        return (0, 0.0);
    }
    ...
    for (c, centre) in centres.iter().enumerate() {
        ...
        let c_norm = infer_compute::norm(&centre_arr.view());
        if c_norm < 1e-8 {
            continue;
        }
        let sim = infer_compute::dot(&direction.view(), &centre_arr.view()) / (d_norm * c_norm);
        if sim > best_sim {
            best_sim = sim;
            best_c = c;
        }
