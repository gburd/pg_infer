...
//! its top-K right singular vectors via subspace iteration.
//!
//! Output layout per expert: `[K, hidden_size] f32` rows = right singular
//! vectors of `gate_proj` (treating `gate_proj` as `out × hidden`).
//!
//! The implementation is LAPACK-free — uses `ndarray` + `rand` only.
//! Speed comes from `rayon` parallelism across experts.
...
/// Summarize a set of expert gate matrices into compact SVD representations.
///
/// `experts` is a slice of per-expert gate weight matrices (each shape `[out_features, hidden]`).
/// Returns `[num_experts, top_k, hidden]` as a Vec of Array2 (one per expert).
///
/// `top_k`: number of singular vectors to retain per expert.
/// `p_iters`: number of power iterations (3-5 for typical models).
pub fn summarize_experts(
    experts: &[Array2<f32>],
    top_k: usize,
    p_iters: usize,
) -> Vec<Array2<f32>> {
    ...
    experts.par_iter().enumerate().map(|(i, expert)| {
        let seed = 42u64.wrapping_add(i as u64);
        top_k_right_singular_vectors(expert.view(), top_k, p_iters, seed)
    }).collect()
}
...
mod tests {
    ...
    #[test]
    fn summarize_multiple_experts() {
        let experts: Vec<Array2<f32>> = (0..4)
            .map(|_| Array2::<f32>::from_shape_fn((16, 8), |(i, j)| (i * 8 + j) as f32 / 128.0))
            .collect();
        let summaries = summarize_experts(&experts, 2, 3);
