...
pub fn build_vindex_streaming(
    ...
    // Detect architecture
    let arch = infer_models::detect_architecture(model_dir)
    ...
    for layer in 0..num_layers {
        ...
        if expert_format == infer_models::ExpertFormat::PackedMxfp4 {
        ...
        } else if expert_format == infer_models::ExpertFormat::PackedBF16 && is_moe {
    ...
    for (layer, layer_down_meta) in all_down_meta.iter_mut().enumerate().take(num_layers) {
        ...
        let down_matrices: Vec<Array2<f32>> = if expert_format == infer_models::ExpertFormat::PackedMxfp4 {
        ...
        for w_down in &down_matrices {
            ...
            for batch_start in (0..num_features).step_by(batch_size) {
                ...
                for feat in batch_start..batch_end {
                    ...
                    let top_k_entries: Vec<infer_models::TopKEntry> = scores.into_iter()
                        .filter_map(|(idx, logit)| {
                            tokenizer.decode(&[idx as u32], true).ok()
                                .map(|s| s.trim().to_string())
                                .filter(|s| !s.is_empty())
                                .map(|token| infer_models::TopKEntry { token, token_id: idx as u32, logit })
                        })
                        .collect();
    ...
    let config = VindexConfig {
        ...
        source: Some(crate::VindexSource {
            ...
            infer_version: env!("CARGO_PKG_VERSION").to_string(),
        }),
        checksums: None,
        extract_level,
        dtype,
