...
pub struct LoadWeightsOptions {
    ...
    /// q_norm / k_norm). Used by `infer serve --ffn-only` — the
...
pub fn load_model_weights_with_opts(
    ...
    if !config.has_model_weights {
        return Err(VindexError::Parse(
            "vindex does not contain model weights. Rebuild with: infer extract-index <model> -o <vindex> --level all".into(),
        ));
    }
    ...
    let arch = infer_models::detect_from_json(&arch_obj);
    ...
    let mut tensors: HashMap<String, infer_models::WeightArray> = HashMap::new();
    let mut vectors: HashMap<String, Vec<f32>> = HashMap::new();
    let mut lm_head_loaded: Option<infer_models::WeightArray> = None;
    ...
    if lm_head_loaded.is_none() && !opts.skip_lm_head {
        ...
        if lm_q4_path.exists() {
            ...
            if let Ok(floats) = infer_models::quant::ggml::dequantize_q4_k(&bytes, padded_floats) {
...
pub fn load_model_weights_q4k(
        ...
        for entry in &entries {
            ...
            if entry.kind == "packed_bf16" {
                ...
                let floats: Option<Vec<f32>> = if entry.kind == "tensor_q4k" {
                    ...
                    infer_models::quant::ggml::dequantize_q4_k(raw_bytes, padded).ok()
                } else {
    ...
    if lm_q4_path.exists() {
        ...
        if let Ok(floats) = infer_models::quant::ggml::dequantize_q4_k(&bytes, padded) {
            if floats.len() >= num_floats {
                if let Ok(arr) = Array2::from_shape_vec(
                    (config.vocab_size, config.hidden_size),
                    floats[..num_floats].to_vec(),
