...
## Q4 Gate KNN (with infer-compute Metal backend)
...
**Build pipeline uses `infer_compute::cpu::ops::q4_common` quantizers as single source of truth.** This ensures the format matches what Metal/CPU kernels expect.
...
Quantizers: all from infer_compute (single source of truth, ADR-008)
```
