...
// via COMPILE INTO VINDEX (see infer-lql) — produces a real
...
│   └── quant/mod.rs            Re-exports from infer_models::quant
│
...
│   ├── kmeans.rs               k-means clustering (BLAS via infer-compute)
...
All matrix operations go through `infer-compute` (BLAS on CPU, Metal GPU planned for gate KNN).
...
`infer_compute::cpu::ops::linalg::ridge_decomposition_solve` with the
MEMIT interpretation:

```rust
use infer_vindex::{memit_solve, MemitFact, MemitStore};
...
pipeline), use `infer-inference::forward::memit`.

| Run | Command |
|-----|---------|
| Demo | `cargo run --release -p infer-vindex --example demo_memit_solve` |
| Bench | `cargo bench -p infer-vindex --bench memit_solve` |
...
`infer_inference::forward::ple::precompute_per_layer_inputs` and
...
```bash
cargo test -p infer-vindex                                                      # 106 tests (lib + 1 integration + doc)

# Demos (synthetic fixtures, no model download needed)
cargo run -p infer-vindex --example demo_features                               # Feature showcase (build, KNN, patches, MoE, f16)
cargo run --release -p infer-vindex --example mmap_demo                         # mmap RAM behaviour + scaling table
cargo run --release -p infer-vindex --example q4k_demo                          # Streaming Q4_K showcase: size comparison, file layout, dequant round-trip
cargo run --release -p infer-vindex --example demo_memit_solve                  # MEMIT closed-form decomposition + MemitStore round-trip

# Criterion benches (run with --quick for a fast sweep, omit for full sample)
cargo bench  -p infer-vindex --bench vindex_ops                                 # KNN, walk, save/load, mutate, MoE
cargo bench  -p infer-vindex --bench vindex_scaling                             # Production dims (CPU)
cargo bench  -p infer-vindex --features metal --bench vindex_scaling            # Production dims (Metal)
cargo bench  -p infer-vindex --bench memit_solve                                # Ridge decomposition throughput
cargo bench  -p infer-vindex --bench extract_throughput                         # Streaming extract: f32 vs Q4K write-path time
cargo bench  -p infer-vindex --bench q4k_vs_f32                                 # Per-layer attn retrieval: mmap memcpy vs mmap + dequant

# Streaming build (one-shot, skips f32 intermediate)
infer extract-index <model> -o <vindex> --quant q4k                             # Q4_K/Q6_K attn + FFN + norms + lm_head in one pass

# Multi-tier build pipeline (post-hoc, uses infer-compute quantizers on an
# already-extracted f32 vindex — kept for backwards compatibility)
cargo run --release -p infer-vindex --example build_q4k_weights -- <vindex>     # Q4_K/Q6_K attn + FFN
cargo run --release -p infer-vindex --example build_attn_q8 -- <vindex>         # Q8 attention (fallback)
cargo run --release -p infer-vindex --example build_interleaved -- <vindex>     # Pack gate|up|down
cargo run --release -p infer-vindex --example build_down_features -- <vindex>   # Feature-major transpose
cargo run --release -p infer-vindex --example build_up_features -- <vindex>     # f16 → f32 decode
cargo run --release -p infer-vindex --example build_gate_q4 -- <vindex>         # Q4 gate vectors
cargo run --release -p infer-vindex --example build_lm_head_q4 -- <vindex>      # Q4 logits projection
```
...
infer-vindex` (full sample) or `-- --quick` (5-iter sweep). HTML
...
6. **All compute through infer-compute** — BLAS dispatch, no raw ndarray .dot() calls
