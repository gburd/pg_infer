...
//!
//! Run: `cargo bench -p infer-vindex --bench q4k_vs_f32`
...
fn bench_q4k_vs_f32(c: &mut Criterion) {
    ...
    let root = std::env::temp_dir().join("infer_bench_q4k_vs_f32");
    let _ = std::fs::remove_dir_all(&root);
    let model_dir = root.join("synth_model");
    make_model(&model_dir, hidden, intermediate, num_layers, vocab);

    let tokenizer = infer_vindex::tokenizers::Tokenizer::from_bytes(MINIMAL_TOKENIZER).unwrap();
    ...
    let mut q4k_index = infer_vindex::VectorIndex::load_vindex(&q4k_dir, &mut lcb).unwrap();
    ...
    group.bench_with_input(
        ...
        |b, _| {
            b.iter(|| {
                ...
                let floats = infer_vindex::config::dtype::decode_floats(
                    bytes,
                    infer_vindex::StorageDtype::F32,
                );
    ...
    group.bench_with_input(
        ...
        |b, _| {
            b.iter(|| {
                ...
                let floats =
                    infer_models::quant::ggml::dequantize_q4_k(bytes, padded).unwrap();
                criterion::black_box(floats);
            });
        },
    );
