...
//!
//! Run: `cargo bench -p infer-vindex --bench vindex_ops`
//!
...
fn bench_save_load(c: &mut Criterion) {
    ...
    group.bench_function("save_gate_vectors", |b| {
        let dir = std::env::temp_dir().join("infer_vindex_bench_save_gate");
        b.iter(|| {
    ...
    group.bench_function("save_down_meta", |b| {
        let dir = std::env::temp_dir().join("infer_vindex_bench_save_meta");
        b.iter(|| {
            let _ = std::fs::remove_dir_all(&dir);
            std::fs::create_dir_all(&dir).unwrap();
    ...
    let load_dir = std::env::temp_dir().join("infer_vindex_bench_load");
