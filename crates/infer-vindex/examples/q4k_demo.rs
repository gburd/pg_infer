...
//!
//! Run: cargo run --release -p infer-vindex --example q4k_demo
...
fn main() {
    println!("=== infer-vindex: streaming Q4_K demo ===\n");

    let tmp = std::env::temp_dir().join("infer_q4k_demo");
    ...
    let cfg = infer_vindex::load_vindex_config(&out_q4k).unwrap();
    ...
    let mut index = infer_vindex::VectorIndex::load_vindex(&out_q4k, &mut lcb).unwrap();
    ...
    let dequant = infer_models::quant::ggml::dequantize_q4_k(q_bytes, padded).unwrap();
    ...
    let v_dequant = infer_models::quant::ggml::dequantize_q6_k(v_bytes, padded).unwrap();
