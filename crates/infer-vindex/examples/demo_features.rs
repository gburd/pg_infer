//! Vindex Feature Showcase — demonstrates the complete infer-vindex API.
//!
...
//!
//! Run: cargo run -p infer-vindex --example vindex_demo

use infer_models::TopKEntry;
use infer_vindex::{FeatureMeta, VectorIndex, VindexConfig};
...
fn main() {
    ...
    for &(family, layers) in &[
        ...
        match infer_vindex::LayerBands::for_family(family, layers) {
    ...
    let dir = std::env::temp_dir().join("infer_vindex_showcase");
    ...
    let config = make_config("showcase", 2, 4, 5, layer_infos, infer_vindex::StorageDtype::F32);
    VectorIndex::save_config(&config, &dir).unwrap();

    if let Some(ref checksums) = config.checksums {
        let results = infer_vindex::format::checksums::verify_checksums(&dir, checksums).unwrap();
        print!("  Checksums: ");
    ...
    let lc = infer_vindex::load_vindex_config(&dir).unwrap();
    ...
    let dir16 = std::env::temp_dir().join("infer_vindex_showcase_f16");
    ...
    let f16_bytes = infer_models::quant::half::encode_f16(gate_data);
    ...
    let decoded = infer_models::quant::half::decode_f16(&f16_bytes);
    ...
    let dir_ext = std::env::temp_dir().join("infer_vindex_showcase_extract");
    ...
    let mut ecb = infer_vindex::SilentBuildCallbacks;
    ...
    let ext_config = infer_vindex::load_vindex_config(&dir_ext).unwrap();
    ...
    let dir_p = std::env::temp_dir().join("infer_vindex_showcase_patch");
    ...
    let patch = infer_vindex::VindexPatch {
        ...
        operations: vec![
            infer_vindex::PatchOp::Insert {
                ...
                gate_vector_b64: Some(infer_vindex::patch::core::encode_gate_vector(&[0.0, 0.0, 0.0, 10.0])),
    ...
    let loaded_patch = infer_vindex::VindexPatch::load(&vlp_path).unwrap();
    ...
    println!("  LabelSource: probe={}, cluster={}, pattern={}, none={}",
        infer_vindex::LabelSource::Probe,
        infer_vindex::LabelSource::Cluster,
        infer_vindex::LabelSource::Pattern,
        infer_vindex::LabelSource::None);
    ...
    for (gguf_key, expected) in &keys {
        let normalized = infer_models::loading::gguf::normalize_gguf_key(gguf_key);
...
STAGE edge
  EXPOSE browse
"#;
    let vf = infer_vindex::vindexfile::parse_vindexfile_str(vf_str).unwrap();
    println!("  Directives: {}", vf.directives.len());
    println!("  Stages: {}", vf.stages.len());
    for d in &vf.directives {
        match d {
            infer_vindex::VindexfileDirective::From(p) => println!("    FROM {}", p),
            infer_vindex::VindexfileDirective::Patch(p) => println!("    PATCH {}", p),
            infer_vindex::VindexfileDirective::Insert { entity, relation, target } =>
                println!("    INSERT ({}, {}, {})", entity, relation, target),
            infer_vindex::VindexfileDirective::Delete { entity, relation, target } =>
                println!("    DELETE entity={} relation={} target={}", entity, relation, target),
            infer_vindex::VindexfileDirective::Labels(p) => println!("    LABELS {}", p),
            infer_vindex::VindexfileDirective::Expose(levels) => println!("    EXPOSE {:?}", levels),
        }
    }
    ...
    for (path, expected) in &hf_paths {
        let is_hf = infer_vindex::is_hf_path(path);
    ...
    println!("  CLI: infer hf download user/repo [-o local/]");
    println!("  CLI: infer hf publish ./local.vindex --repo user/repo");
    ...
    let f16_encoded = infer_models::quant::half::encode_f16(&f16_vals);
    let f16_decoded = infer_models::quant::half::decode_f16(&f16_encoded);
    ...
    // bf16 round-trip
    let bf16_encoded = infer_models::quant::half::encode_bf16(&f16_vals);
    let bf16_decoded = infer_models::quant::half::decode_bf16(&bf16_encoded);
    ...
    let q4_result = infer_models::quant::ggml::dequantize(&q4_block, 2, 32).unwrap();
    ...
    let q8_result = infer_models::quant::ggml::dequantize(&q8_block, 6, 32).unwrap();
    ...
    let mxfp4_result = infer_models::quant::mxfp4::dequantize_expert(
    ...
    for exp in [0u8, 125, 126, 127, 128, 129, 130] {
        let val = infer_models::quant::mxfp4::e8m0_to_f32(exp);
        print!("{exp}→{val} ");
    }
    println!("✓");
    ...
    println!("  GGML types: F32={}, F16={}, Q4_0={}, Q8_0={}",
        infer_models::quant::ggml::type_name(0),
        infer_models::quant::ggml::type_name(1),
        infer_models::quant::ggml::type_name(2),
        infer_models::quant::ggml::type_name(6));
...
fn make_config(model: &str, layers: usize, hidden: usize, intermediate: usize,
    ...
    VindexConfig {
        ...
        source: Some(infer_vindex::VindexSource {
            ...
            infer_version: env!("CARGO_PKG_VERSION").into(),
        }),
        checksums: infer_vindex::format::checksums::compute_checksums(
            &std::env::temp_dir().join("infer_vindex_showcase")).ok(),
        num_layers: layers, hidden_size: hidden, intermediate_size: intermediate,
        vocab_size: 200, embed_scale: 1.0,
        extract_level: infer_vindex::ExtractLevel::Browse, dtype,
