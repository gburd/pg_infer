...
//! `crates/infer-vindex/benches/{vindex_ops,vindex_scaling}.rs`.
//!
...
//!
//! Run: `cargo run --release -p infer-vindex --example mmap_demo`
...
fn main() {
    ...
    println!("(For raw timings see `cargo bench -p infer-vindex`)\n");
    ...
    let dir = std::env::temp_dir().join("infer_vindex_mmap_demo");
