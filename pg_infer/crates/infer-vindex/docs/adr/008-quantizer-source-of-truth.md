...
**Status**: Accepted  
**Date**: 2026-04-07  
**Context**: Found critical bug — vindex `build_q4k_weights` reimplemented Q4_K quantization with different formulas than `infer_compute::cpu::ops::q4_common::quantize_q4_k()`. Weights built by vindex would not dequantize correctly through compute shaders.

## Decision

**All quantization functions live in `infer_compute::cpu::ops::q4_common`.** Vindex build tools import from compute, never reimplement.
...
- Build pipeline: vindex examples depend on `infer-compute` (already a dependency)
