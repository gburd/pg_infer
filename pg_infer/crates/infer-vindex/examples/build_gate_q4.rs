...
//!
//! 7x smaller gate files for fast Q4 KNN via infer-compute.
//! The Q4 file is scored directly by the compute crate's Q4 matvec shader.
//!
//! Usage:
//!   cargo run --release -p infer-vindex --example build_gate_vectors_q4 -- <vindex_dir>
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (layer, (num_features, num_floats)) in layers_info.iter().enumerate() {
        ...
        let f32_data: Vec<f32> = if dtype == "f16" {
            infer_models::quant::half::decode_f16(raw)
        } else {
            unsafe {
