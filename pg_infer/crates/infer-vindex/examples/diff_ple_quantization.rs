...
//!
//! Usage: `cargo run --release -p infer-vindex \
...
fn main() {
    ...
    let dense = infer_models::load_model_dir(&model_dir).expect("dense load");
    ...
    let mut q4k = infer_vindex::load_model_weights_q4k(&vindex_dir, &mut cb).expect("q4k load");
    ...
    let mut attn_cb = infer_vindex::SilentLoadCallbacks;
    let mut index = infer_vindex::VectorIndex::load_vindex(&vindex_dir, &mut attn_cb).expect("vindex load");
    ...
    for layer in [0usize, 10] {
        ...
        let dequant = |(bytes, fmt): (&[u8], &str), rows: usize, cols: usize| {
            ...
            let floats = match fmt {
                "Q4_K" => infer_models::quant::ggml::dequantize_q4_k(bytes, padded).unwrap(),
                "Q6_K" => infer_models::quant::ggml::dequantize_q6_k(bytes, padded).unwrap(),
