...
/// All compute goes through infer-compute.
...
impl VectorIndex {
    ...
    ///
    /// Typical use: `infer serve --max-gate-cache-layers N` to bound a
                ...
                let data = match self.gate_mmap_dtype {
                    ...
                    crate::config::dtype::StorageDtype::F16 => {
                        ...
                        if miss {
                            ...
                            cache[layer] = Some(infer_models::quant::half::decode_f16(raw));
                        }
    ...
    pub fn gate_knn_adaptive(
        ...
        if let Some(q4_data) = residency.pinned_q4(layer) {
            if backend.has_q4() {
                ...
                let (q8_x, q8_scales) = infer_compute::cpu::q4::quantize_to_q8(x);
                let num_features = self.num_features(layer);
    ...
    pub fn gate_knn_q4(
        ...
        let (q8_x, q8_scales) = infer_compute::cpu::q4::quantize_to_q8(residual.as_slice().unwrap());
...
mod gate_cache_lru_tests {
    ...
    fn f16_mmap_index(num_layers: usize, num_features: usize, hidden: usize) -> VectorIndex {
        ...
        for l in 0..num_layers {
            ...
            let bytes = infer_models::quant::half::encode_f16(&data);
