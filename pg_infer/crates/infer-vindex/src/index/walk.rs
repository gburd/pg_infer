...
impl VectorIndex {
    ...
    pub fn load_down_features(&mut self, dir: &std::path::Path) -> Result<(), VindexError> {
        ...
        if !path.exists() {
            return Err(VindexError::Parse(
                "down_features.bin not found. Run: cargo run --release -p infer-vindex --example build_down_features -- <vindex>".into()
            ));
        }
    ...
    pub fn load_up_features(&mut self, dir: &std::path::Path) -> Result<(), VindexError> {
        ...
        if !path.exists() {
            return Err(VindexError::Parse(
                "up_features.bin not found. Run: cargo run --release -p infer-vindex --example build_up_features -- <vindex>".into()
            ));
        }
        let file = std::fs::File::open(&path)?;
    ...
    pub fn load_interleaved(&mut self, dir: &std::path::Path) -> Result<(), VindexError> {
        ...
        if !path.exists() {
            return Err(VindexError::Parse(
                "interleaved.bin not found. Run: cargo run --release -p infer-vindex --example build_interleaved -- <vindex>".into()
            ));
        }
        let file = std::fs::File::open(&path)?;
    ...
    fn dequant_q4_matrix(&self, layer: usize, component: usize) -> Option<ndarray::Array2<f32>> {
        ...
        let floats = infer_models::quant::ggml::dequantize_q4_0(q4_data, floats_per_matrix).ok()?;
        ...
        -> Option<std::sync::Arc<Vec<f32>>>
    {
        ...
        let decoded = match format {
            "Q4_K" => infer_models::quant::ggml::dequantize_q4_k(bytes, padded).ok()?,
            "Q6_K" => infer_models::quant::ggml::dequantize_q6_k(bytes, padded).ok()?,
            _ => return None,
        };
    ...
    pub fn q4k_matmul_transb(
        ...
        y_t.par_chunks_mut(x_rows).enumerate().for_each(|(j, slot)| {
            ...
            for i in 0..x_rows {
                ...
                slot[i] = match format {
                    "Q4_K" => infer_models::quant::ggml::q4k_row_dot(w_row, x_row).unwrap_or(0.0),
                    "Q6_K" => infer_models::quant::ggml::q6k_row_dot(w_row, x_row).unwrap_or(0.0),
                    _ => 0.0,
                };
            }
        });
    ...
    pub fn q4k_ffn_row_dot(
        ...
        match format {
            "Q4_K" => {
                ...
                infer_models::quant::ggml::q4k_row_dot(&bytes[start..end], x).ok()
            }
            "Q6_K" => {
                if !hidden.is_multiple_of(256) { return None; }
                ...
                let end = start + bytes_per_row;
                if end > bytes.len() { return None; }
                infer_models::quant::ggml::q6k_row_dot(&bytes[start..end], x).ok()
            }
            _ => None,
        }
    }
    ...
    pub fn q4k_ffn_row_scaled_add(
        ...
        match format {
            "Q4_K" => {
                ...
                infer_models::quant::ggml::q4k_row_scaled_add(&bytes[start..end], alpha, out).is_ok()
            }
            "Q6_K" => {
                if !hidden.is_multiple_of(256) { return false; }
                let bytes_per_row = (hidden / 256) * 210;
                ...
                let end = start + bytes_per_row;
                if end > bytes.len() { return false; }
                infer_models::quant::ggml::q6k_row_scaled_add(&bytes[start..end], alpha, out).is_ok()
            }
            _ => false,
        }
    }
    ...
    pub fn q4k_ffn_row_into(
        ...
        match format {
            "Q4_K" => {
                ...
                match infer_models::quant::ggml::dequantize_q4_k(row_bytes, hidden) {
            ...
            "Q6_K" => {
                ...
                if end > bytes.len() { return false; }
                let row_bytes = &bytes[start..end];
                match infer_models::quant::ggml::dequantize_q6_k(row_bytes, hidden) {
                    Ok(v) => { out.copy_from_slice(&v[..hidden]); true }
                    Err(_) => false,
                }
            }
    ...
    // ── Q4 gate vectors for fast KNN via infer-compute ──
