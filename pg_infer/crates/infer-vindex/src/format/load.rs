...
impl VectorIndex {
    ...
    pub fn load_vindex_with_range(
        ...
        let (gate_mmap, gate_slices, gate_dtype) = if gate_path.exists() {
            ...
            // `infer slice --preset client`): the client runs attention
...
> {
    ...
    for info in &config.layers {
        ...
        let gate_f32 = infer_models::quant::ggml::dequantize_q4_k(q_bytes, padded)
            .map_err(|e| VindexError::Parse(format!("dequantize layer {}: {e}", info.layer)))?;
        let gate_f16_bytes = infer_models::quant::half::encode_f16(&gate_f32[..n]);
