...
|--------|--------|-------|-------|
| **infer Q4_KF decode (34L, KV)** | **8.5ms** | **117** | **Synthetic ceiling** |
| infer Q4_K decode (21L, KV) | 11.6ms | 86 | |
| infer Q8 decode (21L, KV) | 19.3ms | 52 | |
| Ollama (34L) | 10.3ms | 98 | |
| **vs Ollama (synthetic)** | **0.83x** | — | **17% faster** |

### Real vindex (infer bench, gemma3-4b-q4k-v2.vindex, 2026-04-19)

Prompt: "The capital of France is" (5 tokens), 50 tok, 3 warmup.

| Engine | prefill | ms/tok | tok/s | Notes |
|--------|---------|--------|-------|-------|
| **infer Metal** | **67.7ms** | **15.6ms** | **64.1** | |
...
- See infer-compute ADR-009

## Connection to Compute and Vindex

```
infer-inference orchestrates:
  predict_honest()
