...
//!
//! Usage:
//!   cargo run --release -p infer-inference --example bench_gemma4 [-- model_name]
//!   Default: google/gemma-4-E2B-it

use std::time::Instant;
use ndarray::Array2;

use infer_inference::attention::{apply_rope, apply_rope_partial, gqa_attention_with_weights};
use infer_inference::forward::{embed_tokens_pub, apply_norm, dot_proj, predict, forward_to_layer};
use infer_inference::residual::{rms_norm_heads, rms_norm_heads_no_weight};
use infer_models::{load_model_dir, resolve_model_path};
            ...
            if has_gemma4 {
                ...
                match result {
                    Ok(out) => {
                        ...
                        println!("  infer time:  {:.0} ms", full_us / 1000.0);
                        let ratio = ollama_ms as f64 / (full_us / 1000.0);
                        println!("  Ratio: Ollama is {ratio:.1}x vs infer (CPU f32)");
                    }
