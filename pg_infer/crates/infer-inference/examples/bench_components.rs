...
//!
//! Run: cargo run --release -p infer-inference --example bench_components
...
fn main() {
    ...
    println!("\n--- Comparison ---\n");
    println!("  infer CPU (projected):  {:.1}ms  ({:.0} tok/s)", full_model / 1000.0, 1_000_000.0 / full_model);
    println!("  infer GPU Q4_K decode:  17.5ms  (57 tok/s)");
