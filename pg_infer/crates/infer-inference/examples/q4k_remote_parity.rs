...
//! `infer serve --ffn-only` on the same vindex. Asserts:
//!
...
//! cargo run --release -p infer-cli -- serve path/to/gemma4-31b-q4k.vindex \
//!   --port 8088 --ffn-only \
...
//! cargo run --release -p infer-inference --example q4k_remote_parity -- \
...
use infer_inference::ffn::{RemoteFfnConfig, RemoteWalkBackend};
use infer_inference::vindex::{predict_q4k, predict_q4k_with_ffn};
use infer_vindex::{
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let token_ids = infer_inference::encode_prompt(&tokenizer, &*weights_local.arch, &prompt)
    ...
    let remote = RemoteWalkBackend::connect(remote_config)
        .map_err(|e| format!("remote connect failed ({server_url}): {e}\n\
                              → is `infer serve {} --ffn-only` running on {server_url}?",
                              vindex_path.display()))?;
    assert_eq!(
        remote.hidden_size(),
...
fn print_usage() {
    eprintln!(
        "Usage: q4k_remote_parity \
         ...
         [--tolerance 1e-4]\n\
         \n\
         Requires a running `infer serve <vindex> --port <port> --ffn-only` \
