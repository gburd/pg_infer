...
//!
//! Run:
//!   cargo run --release -p infer-inference --example profile_walk_accuracy -- \
...
fn main() {
    ...
    let model = infer_inference::InferenceModel::load(model_path).unwrap();
    let weights = model.weights();
    let vindex_dir = std::path::PathBuf::from(vindex_path);
    let mut index = infer_vindex::VectorIndex::load_vindex(
        &vindex_dir, &mut infer_vindex::SilentLoadCallbacks,
    ).unwrap();
    ...
    let mut h = infer_inference::forward::embed_tokens_pub(weights, &token_ids);
    for layer in 0..14 {
        let (h_pa, _, _) = infer_inference::attention::run_attention_block_gpu(
            weights, &h, layer, false, None,
        ).unwrap();
        let dense_ffn = infer_inference::WeightFfn { weights };
        let (h_out, _) = infer_inference::forward::run_ffn(weights, &h_pa, layer, &dense_ffn, false);
        h = h_out;
    }

    // Get the post-attention state at L14
    let (h_post_attn, _, _) = infer_inference::attention::run_attention_block_gpu(
        weights, &h, 14, false, None,
    ).unwrap();

    // Dense FFN output (ground truth)
    let dense_ffn = infer_inference::WeightFfn { weights };
    let (dense_out, _) = infer_inference::forward::run_ffn(weights, &h_post_attn, 14, &dense_ffn, false);
    let dense_row = dense_out.row(dense_out.shape()[0] - 1);
    let dense_norm = infer_compute::norm(&dense_row);

    // Count non-zero activations in dense path
    let norm_offset = weights.arch.norm_weight_offset();
    let h_ffn = infer_inference::forward::apply_norm(
        weights, &h_post_attn,
    ...
    let mut activations: Vec<f32> = gate_scores.iter().zip(up_scores.iter())
        .map(|(&g, &u)| {
            let act_g = infer_inference::ffn::gelu_tanh(g);
            act_g * u
        })
        .collect();
    ...
    println!("  ───── ──────── ────────── ──────── ──────────");
    let _gate_index: &dyn infer_vindex::GateIndex = &index;
    for k in [10, 50, 100, 200, 500, 1000, 2000, 4000, 8192, intermediate] {
        let walk_ffn = infer_inference::vindex::WalkFfn::new(weights, &index, k);
        let t = std::time::Instant::now();
        let (walk_out, _) = infer_inference::forward::run_ffn(weights, &h_post_attn, 14, &walk_ffn, false);
        let walk_ms = t.elapsed().as_secs_f64() * 1000.0;

        let walk_row = walk_out.row(walk_out.shape()[0] - 1);
        let walk_norm = infer_compute::norm(&walk_row);

        let cosine = infer_compute::dot(&dense_row, &walk_row) / (dense_norm * walk_norm + 1e-10);
    ...
    // Dense baseline
    let dense_result = infer_inference::predict(weights, model.tokenizer(), &token_ids, 5);
    ...
    for k in [50, 200, 500, 2000, 8192] {
        let walk_ffn = infer_inference::vindex::WalkFfn::new(weights, &index, k);
        let walk_graph = infer_inference::WalkLayerGraph { ffn: &walk_ffn, backend: None };
        let result = infer_inference::predict_with_graph(
