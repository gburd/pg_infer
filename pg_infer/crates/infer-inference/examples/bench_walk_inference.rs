...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let h = infer_inference::forward_to_layer(weights, &token_ids, 0);
    let h_norm = infer_inference::ndarray::Array2::from_shape_fn(
    ...
    let t0 = Instant::now();
    for _ in 0..3 {
        use infer_inference::FfnBackend;
    ...
    let t0 = Instant::now();
    for _ in 0..3 {
        let _ = infer_inference::ffn::sparse_compute::sparse_ffn_forward(weights, 0, &h_norm, &features);
    }
