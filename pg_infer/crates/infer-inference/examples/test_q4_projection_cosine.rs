...
//!
//! Usage:
//!   cargo run --release -p infer-inference --example test_q4_projection_cosine

extern crate blas_src;

use infer_inference::{InferenceModel, forward::forward_to_layer};
use infer_models::quant::ggml::{quantize_q4_0, dequantize_q4_0};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &layer in &[0, 10, 13, 15, 20, 33] {
        ...
        if let Some(w) = weights.tensors.get(&key) {
            ...
            // Q8
            let (w_q8, w_scales) = infer_compute::cpu::ops::q8_matvec::quantize_weights_q8(w_data, rows, cols);
            let (x_q8, x_scales) = infer_compute::cpu::ops::q4_common::quantize_to_q8(x);
            let q8_result = infer_compute::cpu::ops::q8_matvec::dispatch(
