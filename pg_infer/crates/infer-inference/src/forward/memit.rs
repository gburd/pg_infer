...
fn memit_solve_layer(
    ...
    let l = infer_compute::cpu::ops::linalg::cholesky(&cov_f64, ridge)
    ...
    let c_inv_kt = infer_compute::cpu::ops::linalg::cholesky_solve(&l, &k_t); // [ffn_dim × N]
    ...
    let l_s = infer_compute::cpu::ops::linalg::cholesky(&s, 0.0)
        .map_err(|e| format!("MEMIT: S matrix Cholesky failed — {e}"))?;

    // ΔW = R^T @ S⁻¹ @ Q  [hidden × ffn_dim]
    //     = R^T @ (S⁻¹ Q)
    let s_inv_q = infer_compute::cpu::ops::linalg::cholesky_solve(&l_s, &q); // [N × ffn_dim]
