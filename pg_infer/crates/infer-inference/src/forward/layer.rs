...
pub fn run_ffn(
    ...
    // Layer-0 stage dumps (INFER_CPU_STAGE_DUMP=<dir>) — matches the
    // Metal `INFER_METAL_DUMP_LAYERS` convention. Lets us diff per-stage
    // intermediates between CPU and Metal for the first layer.
    let stage_dump_dir = if layer == 0 { std::env::var("INFER_CPU_STAGE_DUMP").ok() } else { None };
