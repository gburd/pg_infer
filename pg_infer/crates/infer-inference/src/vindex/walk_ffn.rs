...
use infer_vindex::{GateIndex, WalkHit, WalkTrace};
...
impl<'a> WalkFfn<'a> {
    ...
    fn walk_ffn_sparse(
        ...
        if !layer_has_overrides && is_gated && k_is_full {
                ...
                (self.index.gate_scores_batch_backend(layer, x, self.backend), x_slice_for_matmul)
            {
                ...
                let up_scores: Option<ndarray::Array2<f32>> = if let Some(v) = up_native {
                    Some(infer_compute::dot_proj_gpu(x, &v, self.backend))
                ...
                if let Some(up_scores) = up_scores {
                    ...
                    let out_matmul: Option<ndarray::Array2<f32>> = if let Some(v) = down_native {
                        Some(infer_compute::matmul_gpu(&activation, &v, self.backend))
        ...
        for s in 0..seq_len {
            ...
            // calls `infer_models::quant::ggml::q4k_row_dot` directly (no
                    ...
                    .map(|chunk| {
                        ...
                        for &(feat, gate_score) in chunk {
                            let up_score = if let Some(up_view) = up_native_ref {
                                ...
                                infer_models::quant::ggml::q4k_row_dot(
    ...
    fn walk_ffn_q4_interleaved(
        ...
        use infer_compute::cpu::ops::{q4_matvec, q4_vecmat};
    ...
    fn walk_ffn_interleaved(
        ...
        let gate_scores = infer_compute::dot_proj_gpu(x, &gate_view, self.backend);

        // up_scores = x @ up_vectors^T (contiguous, right after gate in memory)
        let up_scores = infer_compute::dot_proj_gpu(x, &up_view, self.backend);
        ...
        let mut out = infer_compute::matmul_gpu(&activation, &down_view, self.backend);
        ...
        let dequant = |bytes: &[u8], fmt: &str, rows: usize, cols: usize| -> Array2<f32> {
            ...
            let flat = match fmt {
                "Q6_K" => infer_models::quant::ggml::dequantize_q6_k(bytes, padded)
                    .expect("q6k dequant"),
                _ => infer_models::quant::ggml::dequantize_q4_k(bytes, padded)
                    .expect("q4k dequant"),
            };
    ...
    fn walk_ffn_exact(
        ...
        let out = if let Some(down_view) = self.index.down_layer_matrix(layer) {
            ...
            infer_compute::matmul_gpu(&activation, &down_view, self.backend)
        } else {
            ...
            infer_compute::dot_proj_gpu(&activation, w_down, self.backend)
        };
