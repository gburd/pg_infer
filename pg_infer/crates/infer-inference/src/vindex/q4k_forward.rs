...
//! `infer_compute::cpu::ops::q4k_matvec` directly to avoid the per-layer
...
//! `infer-cli/src/commands/extraction/walk_cmd.rs`.
...
fn predict_q4k_hidden(
    ...
    let dump_dir = std::env::var("INFER_CPU_DUMP_LAYERS").ok();
...
/// predictions in the same shape as `infer_inference::predict`.
pub fn predict_q4k(
    weights: &mut ModelWeights,
    tokenizer: &Tokenizer,
    token_ids: &[u32],
...
/// `infer dev walk --predict --max-tokens N` makes for the CPU path. For
...
/// one HTTP round trip to an `infer serve --ffn-only` server.
///
...
pub fn predict_q4k_metal(
    ...
    top_k: usize,
    index: &VectorIndex,
    backend: &dyn infer_compute::ComputeBackend,
) -> PredictResult {
    use infer_compute::QuantFormat;
    ...
    let layers: Vec<_> = (0..num_layers).map(|layer| {
        ...
        let gate = infer_compute::QuantWeight { data: gate_bytes, scales: None, format: to_format(gate_fmt) };
        let up   = infer_compute::QuantWeight { data: up_bytes,   scales: None, format: to_format(up_fmt) };
        let down = infer_compute::QuantWeight { data: down_bytes, scales: None, format: to_format(down_fmt) };
...
///
/// Used by `infer-server`'s `/v1/walk-ffn` (full_output mode) when serving
...
pub fn q4k_ffn_forward_layer(
    ...
    let activation = match arch.activation() {
        infer_models::Activation::GeluTanh | infer_models::Activation::Gelu => {
...
fn dequantize_matrix(bytes: &[u8], format: &str, rows: usize, cols: usize) -> Array2<f32> {
    ...
    let floats = match format {
        "Q4_K" => infer_models::quant::ggml::dequantize_q4_k(bytes, padded)
            .expect("Q4_K dequant failed"),
        "Q6_K" => infer_models::quant::ggml::dequantize_q6_k(bytes, padded)
