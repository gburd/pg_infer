...
pub fn run_attention_with_kv_backend(
    ...
    let (mut q, mut k, mut v) = (
        infer_compute::dot_proj_gpu(&h_norm, wq, backend),
        infer_compute::dot_proj_gpu(&h_norm, wk, backend),
        infer_compute::dot_proj_gpu(&h_norm, wv, backend),
    );
    ...
    let mut o = infer_compute::dot_proj_gpu(&attn_out, wo, backend);
...
pub fn q4_attention_proj(
    ...
    for s in 0..seq_len {
        ...
        let (q8_x, q8_scales) = infer_compute::cpu::q4::quantize_to_q8(x_slice);
