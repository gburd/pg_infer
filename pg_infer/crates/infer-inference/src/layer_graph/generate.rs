...
pub(crate) fn lm_head_topk(
    index: &infer_vindex::VectorIndex,
...
/// Kept for the `INFER_METAL_COMPARE_CPU=1` diagnostic mode which wants a
...
fn cpu_lm_head_topk(
    ...
    backend_lm_head_topk(weights, query, top_k, &infer_compute::CpuBackend)
}
...
pub fn generate(
    ...
    let ffn_format = if ffn_is_q4k { infer_compute::QuantFormat::Q4_K } else { infer_compute::QuantFormat::Q4_0 };
    ...
    let compare = std::env::var("INFER_METAL_COMPARE_CPU").is_ok();
    ...
    // CPU-vs-Metal comparison mode (INFER_METAL_COMPARE_CPU=1). Runs the
    ...
    if compare {
        ...
        eprintln!("[compare] (run `infer walk --predict` (no --metal) for CPU reference tokens)");
    }
    ...
    // Per-stage decode profiling. Set INFER_PROFILE_DECODE=1 to log a
    // one-line per-step breakdown of embed / GPU forward / final norm /
    // lm_head / detokenize, plus a summary at the end.
    let profile = std::env::var("INFER_PROFILE_DECODE").is_ok();
    let profile_split = std::env::var("INFER_PROFILE_SPLIT").is_ok();
    ...
    // keep the struct tiny — the `infer bench` harness averages these
