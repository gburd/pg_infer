...
//! consults `INFER_PROBE_PATH` and `INFER_PROBE_DIR` env overrides before
...
impl CascadeTrie {
    ...
    ///
    /// Search order:
    ///   1. `INFER_PROBE_PATH` env var — used verbatim if set and the file exists.
    ///   2. `INFER_PROBE_DIR` env var, joined with [`Self::filename_for`].
        ...
        P: AsRef<Path>,
    {
        Self::find_with_env(
            model_id,
            std::env::var("INFER_PROBE_PATH").ok().map(PathBuf::from),
            std::env::var("INFER_PROBE_DIR").ok().map(PathBuf::from),
            extra_dirs,
        )
    }
...
mod tests {
    ...
    fn find_returns_none_when_nothing_matches() {
        ...
        let dir = std::env::temp_dir().join("infer-nonexistent-probe-dir-xyz");
