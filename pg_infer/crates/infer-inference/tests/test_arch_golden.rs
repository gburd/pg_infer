...
//! real vindex out of `~/.cache/infer/local/` (or the workspace `output/`
...
//! green and the test is a no-op. Set `INFER_ARCH_STRICT=1` to turn a missing
//! vindex into a hard failure (useful locally to catch "I forgot to pull").
//!
//! **Env knobs.**
//!   - `INFER_ARCH_STRICT=1` — require every case's vindex to be present.
//!   - `INFER_ARCH_PROMPT=<text>` — override the shared prompt (default:
//!     `"The capital of France is"`).
//!   - `INFER_ARCH_TOKENS=<n>` — override the generated-token budget (default 3).
//!
...
use infer_compute::{cpu_backend, default_backend, ComputeBackend};
use infer_inference::encode_prompt;
use infer_inference::layer_graph::{generate as gen, CachedLayerGraph};
use infer_vindex::{
...
/// `~/.cache/infer/local/<name>.vindex/`). `expected_substring` is a
...
/// or a hard error based on `INFER_ARCH_STRICT`.
fn find_vindex(name: &str) -> Option<PathBuf> {
    let filename = format!("{name}.vindex");

    // Absolute-override env var.
    if let Ok(env_path) = std::env::var(format!("INFER_VINDEX_{}", name.to_uppercase().replace('-', "_"))) {
    ...
    let candidates = [
        PathBuf::from(&home).join(".cache/infer/local").join(&filename),
...
fn run_case(
    ...
    let cfg = infer_vindex::load_vindex_config(vindex_path)
...
fn strict_mode() -> bool {
    matches!(std::env::var("INFER_ARCH_STRICT").ok().as_deref(), Some("1") | Some("true"))
}

fn prompt() -> String {
    std::env::var("INFER_ARCH_PROMPT").unwrap_or_else(|_| "The capital of France is".to_string())
}

fn max_tokens() -> usize {
    std::env::var("INFER_ARCH_TOKENS")
        .ok()
...
fn exercise_case(case: &ArchCase, backend_kind: BackendKind) {
    ...
    let Some(vindex_path) = find_vindex(case.vindex_name) else {
        if strict_mode() {
            panic!(
                "[{}/{}] vindex `{}` not found in cache (INFER_ARCH_STRICT=1)",
                case.arch_family, backend_kind.name(), case.vindex_name,
            );
        }
        eprintln!(
            "[{}/{}] skip: vindex `{}` not found in ~/.cache/infer/local/ or output/ — \
             set INFER_ARCH_STRICT=1 to fail instead.",
            case.arch_family, backend_kind.name(), case.vindex_name,
        );
        return;
    };
