...
mod walker_tests {
    ...
    fn test_weight_walker_loads() {
        let dir = std::env::temp_dir().join("infer_test_weight_load");
        let _ = std::fs::remove_dir_all(&dir);
        create_mock_model(&dir);

        let walker =
            infer_inference::walker::weight_walker::WeightWalker::load(dir.to_str().unwrap())
                .unwrap();
    ...
    fn test_weight_walker_extracts_edges() {
        let dir = std::env::temp_dir().join("infer_test_weight_edges");
        let _ = std::fs::remove_dir_all(&dir);
        create_mock_model(&dir);
        ...
        let walker =
            infer_inference::walker::weight_walker::WeightWalker::load(dir.to_str().unwrap())
                .unwrap();
        let config = infer_inference::walker::weight_walker::WalkConfig {
            top_k: 3,
            min_score: 0.0,
        };
        let mut graph = infer_core::Graph::new();
        let mut callbacks = infer_inference::walker::weight_walker::SilentWalkCallbacks;
        ...
        for edge in graph.edges() {
            assert_eq!(edge.source, infer_core::SourceType::Parametric);
    ...
    fn test_weight_walker_all_layers() {
        let dir = std::env::temp_dir().join("infer_test_weight_all");
        let _ = std::fs::remove_dir_all(&dir);
        create_mock_model(&dir);
        ...
        let results = infer_inference::walk_model(
            dir.to_str().unwrap(),
            None,
            &config,
            &mut graph,
    ...
    fn test_weight_walker_layer_stats() {
        let dir = std::env::temp_dir().join("infer_test_weight_stats");
        let _ = std::fs::remove_dir_all(&dir);
        create_mock_model(&dir);
    ...
    fn test_attention_walker_loads() {
        let dir = std::env::temp_dir().join("infer_test_attn_load");
        let _ = std::fs::remove_dir_all(&dir);
        create_mock_model(&dir);

        let walker =
            infer_inference::walker::attention_walker::AttentionWalker::load(dir.to_str().unwrap())
                .unwrap();
        assert_eq!(walker.num_layers(), 2);
    ...
    fn test_attention_walker_extracts_edges() {
        let dir = std::env::temp_dir().join("infer_test_attn_edges");
        let _ = std::fs::remove_dir_all(&dir);
        create_mock_model(&dir);
    ...
    fn test_vector_extractor_ffn_down() {
        let dir = std::env::temp_dir().join("infer_test_vec_down");
        let _ = std::fs::remove_dir_all(&dir);
        create_mock_model(&dir);

        let extractor =
            infer_inference::walker::vector_extractor::VectorExtractor::load(dir.to_str().unwrap())
                .unwrap();
        ...
        let config = infer_inference::walker::vector_extractor::ExtractConfig {
            components: vec!["ffn_down".to_string()],
            layers: Some(vec![0]),
            top_k: 3,
        };
        let mut callbacks = infer_inference::walker::vector_extractor::SilentExtractCallbacks;
    ...
    fn test_vector_extractor_embeddings() {
        let dir = std::env::temp_dir().join("infer_test_vec_embed");
        let _ = std::fs::remove_dir_all(&dir);
        create_mock_model(&dir);
    ...
    fn test_loader_loads_all_tensors() {
        let dir = std::env::temp_dir().join("infer_test_loader");
        let _ = std::fs::remove_dir_all(&dir);
        create_mock_model(&dir);

        let weights = infer_inference::model::load_model_dir(&dir).unwrap();
    ...
    fn test_loader_missing_directory() {
        let result = infer_inference::model::load_model_dir("/nonexistent/path/model");
    ...
    fn test_forward_captures_residuals() {
        let dir = std::env::temp_dir().join("infer_test_forward");
        let _ = std::fs::remove_dir_all(&dir);
        create_mock_model(&dir);
        ...
        let residuals = infer_inference::forward::capture_residuals(&weights, &token_ids, &[0, 1]);
    ...
    fn test_forward_single_token() {
        let dir = std::env::temp_dir().join("infer_test_forward_single");
        let _ = std::fs::remove_dir_all(&dir);
        create_mock_model(&dir);

        let weights = infer_inference::model::load_model_dir(&dir).unwrap();
        let residuals = infer_inference::forward::capture_residuals(&weights, &[0], &[1]);
    ...
    fn test_capture_decoy_residuals_returns_vector_per_prompt() {
        ...
        let dir = std::env::temp_dir().join("infer_test_capture_decoys");
        let _ = std::fs::remove_dir_all(&dir);
        create_mock_model(&dir);
        ...
        let residuals = infer_inference::capture_decoy_residuals(&weights, &prompts, 1);
    ...
    fn test_capture_decoy_residuals_empty_input() {
        let dir = std::env::temp_dir().join("infer_test_capture_decoys_empty");
        let _ = std::fs::remove_dir_all(&dir);
        create_mock_model(&dir);

        let weights = infer_inference::model::load_model_dir(&dir).unwrap();
        let residuals: Vec<_> = infer_inference::capture_decoy_residuals(&weights, &[], 0);
