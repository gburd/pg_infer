//! Integration test for `infer_inference::vindex::generate_q4k_cpu`.
//!
...
//!
//! ```sh
//! cargo test -p infer-inference --test test_generate_q4k_cpu -- --ignored
//! ```
//!
...
//! under `--ignored`. Override the search with `INFER_TEST_VINDEX=<path>`.

use std::path::PathBuf;
use std::time::Instant;

use infer_inference::vindex::generate_q4k_cpu;
use infer_vindex::{
...
fn find_q4k_vindex() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("INFER_TEST_VINDEX") {
    ...
    let candidates = [
        // Workspace output directory (matches `cargo run -p infer-cli build`).
        PathBuf::from("output/gemma3-4b-q4k-v2.vindex"),
        PathBuf::from("output/gemma4-e2b-q4k.vindex"),
        // User cache locations.
        PathBuf::from(&home).join(".cache/infer/local/mistral-7b-v0.1-q4k.vindex"),
        PathBuf::from(&home).join(".cache/infer/local/llama2-7b-q4k.vindex"),
    ];
...
fn generate_q4k_cpu_produces_tokens_against_real_vindex() {
    let Some(vindex_path) = find_q4k_vindex() else {
        eprintln!(
            "skip: no Q4_K vindex found. Set INFER_TEST_VINDEX=<path> to override.",
        );
        return;
    };
    ...
    let prompt_ids = infer_inference::encode_prompt(&tokenizer, &*weights.arch, prompt)
        .expect("tokenize");
