...
fn test_json_file_roundtrip() {
    let g = sample_graph();
    let path = std::env::temp_dir().join("test_roundtrip.infer.json");
    save(&g, &path).unwrap();
...
fn test_json_format_structure() {
    ...
    assert_eq!(json["infer_version"], "0.1.0");
...
fn test_msgpack_file_roundtrip() {
    let g = sample_graph();
    let path = std::env::temp_dir().join("test_roundtrip.infer.bin");
    save(&g, &path).unwrap();
    let restored = load(&path).unwrap();
    assert_graphs_equal(&g, &restored);
    std::fs::remove_file(&path).ok();
...
fn test_format_from_path() {
    assert_eq!(Format::from_path("graph.infer.json"), Some(Format::Json));
    assert_eq!(Format::from_path("graph.json"), Some(Format::Json));
    assert_eq!(
        Format::from_path("graph.infer.bin"),
