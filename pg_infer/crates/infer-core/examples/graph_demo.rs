...
//!
//! Run: cargo run --release -p infer-core --example graph_demo

use infer_core::*;

fn main() {
    println!("=== infer Graph Engine Demo ===\n");
    ...
    // ── Save / Load ──
    println!("\n--- Serialization ---");
    let tmp = std::env::temp_dir().join("graph_demo.infer.json");
    ...
    // MsgPack
    let tmp_bin = std::env::temp_dir().join("graph_demo.infer.bin");
