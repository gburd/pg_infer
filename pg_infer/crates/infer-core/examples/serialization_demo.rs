...
//!
//! Run: cargo run --release -p infer-core --example serialization_demo

use infer_core::*;

fn main() {
    println!("=== infer Serialization Demo ===\n");
    ...
    // ── File save/load ──
    let tmp_json = std::env::temp_dir().join("demo.infer.json");
    let tmp_bin = std::env::temp_dir().join("demo.infer.bin");
    ...
    println!("\nAuto-detect load:");
    println!("  .infer.json → {} edges", g1.edge_count());
    println!("  .infer.bin  → {} edges", g2.edge_count());
