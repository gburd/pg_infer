...
- **Metal shaders** (`q4k_matvec`, `q6k_matvec`): Original infer implementation matching the GGUF dequantization formula.
- **CPU reference** (`q4k_matvec.rs`, `q6k_matvec.rs`): Original infer implementation for cross-backend testing. Scalar code mirroring the Metal shader logic.
- **Quantizers** (`quantize_q4_k`, `quantize_q6_k`, `quantize_q4_k_gguf`): Original infer implementation. `quantize_q4_k_gguf` produces exact GGUF 144-byte blocks with packed 12-byte scales+mins.

## Q4_K Block Layouts

| Field | infer (148B) | GGUF (144B) |
|-------|-------------|-------------|
