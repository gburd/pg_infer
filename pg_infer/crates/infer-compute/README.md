...
Hardware-accelerated compute backends for infer. CPU (BLAS + NEON Q4), Metal GPU, and future CUDA.

## What it does

Provides a `ComputeBackend` trait that abstracts all hardware-specific matrix operations. Every infer crate (inference, vindex) uses this trait — the caller never knows whether the operation runs on CPU or GPU.
...
```
infer Q4_KF (34 layers):       8.5ms/token = 117 tok/s (decode, KV cached)
...
```rust
use infer_compute::{ComputeBackend, default_backend};
...
Beyond the matmul/quantization backends, `infer-compute` ships a small set
...
| `ridge_decomposition_solve(K, T, λ)` | closed-form `ΔW = T^T (K K^T + λI)⁻¹ K`, returns `(d,d)` | `infer_vindex::memit_solve` (COMPACT MAJOR) |
...
Bench: `cargo bench -p infer-compute --bench linalg`
Demo:  `cargo run --release -p infer-compute --example demo_ridge_solve`

> The MEMIT-flavoured wrapper (`memit_solve` returning `MemitSolveResult`
> with per-fact reconstruction quality) lives in `infer-vindex` next to
> `MemitStore`. The production weight-edit pipeline with covariance
> whitening is in `infer-inference/forward/memit.rs`.
...
# CPU + Metal (83 tests)
cargo test -p infer-compute --features metal
```
...
cargo run --release --features metal -p infer-compute --example demo_architecture

# Basic usage — backend detection, matmul, Q4 dispatch
cargo run --release --features metal -p infer-compute --example demo_basic
```

### Benchmarks: Compare (us vs Ollama)

```bash
cargo run --release --features metal -p infer-compute --example compare_decode     # Q4_K vs Q8, KV cached
cargo run --release --features metal -p infer-compute --example compare_generation  # Prefill + decode
cargo run --release --features metal -p infer-compute --example compare_pipeline    # Attention + FFN breakdown
cargo run --release --features metal -p infer-compute --example compare_formats     # Q4_KF vs Q4_K vs GGUF
```

### Benchmarks: Profile (bottleneck analysis)

```bash
cargo run --release --features metal -p infer-compute --example profile_components   # Every op isolated over 34 layers
cargo run --release --features metal -p infer-compute --example profile_operations   # CPU vs Metal per-operation
cargo run --release --features metal -p infer-compute --example profile_kernels      # Q4 v1-v5, sparse, attention
cargo run --release --features metal -p infer-compute --example profile_raw_dispatch # Pure kernel, zero overhead
cargo run --release --features metal -p infer-compute --example profile_new_kernels  # New model-agnostic kernels
cargo run --release --features metal -p infer-compute --example profile_kv_cache     # Attention vs cache length
cargo run --release --features metal -p infer-compute --example profile_bandwidth    # Raw memory throughput
```

### Benchmarks: Best Run

```bash
cargo run --release --features metal -p infer-compute --example best_pipeline       # Full pipeline, 1 cmd buffer
cargo run --release --features metal -p infer-compute --example best_multi_layer     # Multi-layer batch
```
