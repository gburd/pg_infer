...
//!
//! Run with: cargo test -p infer-compute --features metal
...
use ndarray::Array2;
use infer_compute::{ComputeBackend, cpu::q4};
...
fn get_metal() -> infer_compute::metal::MetalBackend {
    infer_compute::metal::MetalBackend::new().expect("Metal device required for these tests")
}
...
fn all_shaders_compile() {
    let src = infer_compute::metal::shaders::all_shaders();
...
fn q8_matvec_metal_nonzero() {
    ...
    let (w_q8, w_scales) = infer_compute::cpu::ops::q8_matvec::quantize_weights_q8(&weights, rows, hidden);
    let (x_q8, x_scales) = infer_compute::cpu::ops::q4_common::quantize_to_q8(&x);

    // CPU reference
    let cpu_result = infer_compute::cpu::ops::q8_matvec::dispatch(&w_q8, &w_scales, &x_q8, &x_scales, rows, hidden);
...
fn sparse_matvec_matches_dense() {
    ...
    let bufs = &infer_compute::metal::buffers::BufferCache::new(&device);
...
fn geglu_matches_cpu() {
    ...
    // CPU reference
    let cpu_result = infer_compute::cpu::ops::geglu::geglu_silu_alloc(&gate, &up);
...
fn rms_norm_large_vector_simd_cooperative() {
    ...
    let metal_result = infer_compute::metal::buffers::read_buffer_f32(&buf_out, len);
    let diff = max_diff(&cpu_result, &metal_result);
...
fn quantize_q8_matches_cpu() {
    ...
    // CPU reference
    let (cpu_q8, cpu_scales) = infer_compute::cpu::q4::quantize_to_q8(&x);
...
fn rms_norm_q8_matches_separate_ops() {
    ...
    let (cpu_q8, cpu_scales) = infer_compute::cpu::q4::quantize_to_q8(&normed);
...
fn q4k_matvec_matches_cpu() {
    let metal = get_metal();
    let cpu = infer_compute::cpu::CpuBackend;
    ...
    let q4k_data = infer_compute::cpu::ops::q4_common::quantize_q4_k(&matrix);
...
fn q6k_matvec_matches_cpu() {
    ...
    let q6k_data = infer_compute::cpu::ops::q4_common::quantize_q6_k(&matrix);
...
fn full_pipeline_seq1_produces_nonzero() {
    ...
    let layer = infer_compute::FullPipelineLayer {
        wq: infer_compute::QuantWeight { data: &wq_data, scales: Some(&q8_s_q), format: infer_compute::QuantFormat::Q4_0 },
        wk: infer_compute::QuantWeight { data: &wk_data, scales: Some(&q8_s_q), format: infer_compute::QuantFormat::Q4_0 },
        wv: infer_compute::QuantWeight { data: &wv_data, scales: Some(&q8_s_q), format: infer_compute::QuantFormat::Q4_0 },
        wo: infer_compute::QuantWeight { data: &wo_data, scales: Some(&q8_s_q), format: infer_compute::QuantFormat::Q4_0 },
        gate: infer_compute::QuantWeight { data: &gate_data, scales: None, format: infer_compute::QuantFormat::Q4_0 },
        up: infer_compute::QuantWeight { data: &up_data, scales: None, format: infer_compute::QuantFormat::Q4_0 },
        down: infer_compute::QuantWeight { data: &down_data, scales: None, format: infer_compute::QuantFormat::Q4_0 },
        input_norm: &norm,
        post_attn_norm: &norm,
        pre_ffn_norm: None,
        post_ffn_norm: None,
        ...
        has_post_norms: false,
            activation: infer_compute::Activation::Silu,
            qk_norm_offset: 0.0,
            eps: 1e-6,
            norm_type: infer_compute::NormType::RmsNorm,
            ffn_type: infer_compute::FfnType::Gated,
...
fn silu_standalone_matches_cpu() {
    ...
    let result = infer_compute::metal::buffers::read_buffer_f32(&output_buf, n);
...
fn layer_norm_matches_cpu() {
    ...
    let result = infer_compute::metal::buffers::read_buffer_f32(&out_buf, n);
    let diff = max_diff(&expected, &result);
...
fn rms_norm_with_different_eps() {
    ...
    let r1 = infer_compute::metal::buffers::read_buffer_f32(&out1, n);
    let r2 = infer_compute::metal::buffers::read_buffer_f32(&out2, n);
    let diff = max_diff(&r1, &r2);
...
fn q6k_single_superblock_matches_dequantize_reference() {
    ...
    for probe_k in [0usize, 1, 2, 15, 16, 31, 32, 127, 128, 200, 255] {
        ...
        let q6k = infer_compute::cpu::ops::q4_common::quantize_q6_k(&row);
        assert_eq!(q6k.len(), 210, "single superblock should be 210 bytes");

        let dequant = infer_models::quant::ggml::dequantize_q6_k(&q6k, hidden).unwrap();
...
fn q6k_multi_row_diagnostic() {
    ...
    let q6k = infer_compute::cpu::ops::q4_common::quantize_q6_k(&matrix);

    // Reference via dequantize_q6_k + CPU gemv.
    let dequant = infer_models::quant::ggml::dequantize_q6_k(&q6k, rows * hidden).unwrap();
    let mut cpu_ref = vec![0.0f32; rows];
    for row in 0..rows {
...
fn q4k_single_superblock_matches_dequantize_reference() {
    ...
    let q4k = infer_compute::cpu::ops::q4_common::quantize_q4_k(&row);
    assert_eq!(q4k.len(), 144, "single superblock should pack into 144 bytes GGUF");

    let dequant = infer_models::quant::ggml::dequantize_q4_k(&q4k, hidden).unwrap();
    let cpu_ref: f32 = (0..hidden).map(|k| dequant[k] * x[k]).sum();
...
fn q4k_multi_row_matches_dequantize_reference() {
    ...
    let q4k = infer_compute::cpu::ops::q4_common::quantize_q4_k(&matrix);
    let dequant = infer_models::quant::ggml::dequantize_q4_k(&q4k, rows * hidden).unwrap();
...
fn geglu_gelu_tanh_no_nan_on_large_gate() {
    ...
    let out = infer_compute::metal::buffers::read_buffer_f32(&out_buf, n);
...
fn q4kf_proj_matches_cpu_reference() {
    ...
    use infer_compute::metal::shaders::q4kf_qkv_proj as q4kf;
    ...
    let metal_out = infer_compute::metal::buffers::read_buffer_f32(&out_buf, rows);
...
fn q4kf_qkv_proj_matches_individual_projections() {
    ...
    let q_quant = infer_compute::cpu::ops::q4_common::quantize_q4_k(&wq);
    let k_quant = infer_compute::cpu::ops::q4_common::quantize_q4_k(&wk);
    let v_quant = infer_compute::cpu::ops::q4_common::quantize_q4_k(&wv);

    // CPU reference: dequant each and gemv against x.
    let q_deq = infer_models::quant::ggml::dequantize_q4_k(&q_quant, q_rows * hidden).unwrap();
    let k_deq = infer_models::quant::ggml::dequantize_q4_k(&k_quant, k_rows * hidden).unwrap();
    let v_deq = infer_models::quant::ggml::dequantize_q4_k(&v_quant, v_rows * hidden).unwrap();
    ...
    let q_metal = infer_compute::metal::buffers::read_buffer_f32(&q_out, q_rows);
    let k_metal = infer_compute::metal::buffers::read_buffer_f32(&k_out, k_rows);
    let v_metal = infer_compute::metal::buffers::read_buffer_f32(&v_out, v_rows);
...
fn qk_norm_matches_cpu_reference() {
    ...
    let metal_out = infer_compute::metal::buffers::read_buffer_f32(&out_buf, num_heads * head_dim);
...
fn q4kf_proj_matches_cpu_on_real_vindex_bytes() {
    ...
    let dequant = infer_models::quant::ggml::dequantize_q4_k(q_bytes, rows * hidden).unwrap();
    let x: Vec<f32> = (0..hidden).map(|i| ((i as f32) * 0.01).sin()).collect();
    let mut cpu_out = vec![0.0f32; rows];
    for row in 0..rows {
        cpu_out[row] = (0..hidden).map(|k| dequant[row * hidden + k] * x[k]).sum();
...
fn stage_post_attn_pre_norm_matches_cpu() {
    ...
    infer_compute::metal::stages::residual::encode_post_attn(
...
fn stage_post_ffn_pre_norm_matches_cpu() {
    ...
    let enc = cmd.new_compute_command_encoder();
    let mut scratch = |n: u64| bufs.output(n);
    infer_compute::metal::stages::residual::encode_post_ffn(
...
fn stage_quant_matvec_routes_format_to_correct_shader() {
    ...
    let pipes = infer_compute::metal::stages::quant_matvec::Pipelines {
    ...
    // Q4_K route.
    let w_q4k = infer_compute::cpu::ops::q4_common::quantize_q4_k(&w_f32);
    let w_q4k_buf = bufs.get_bytes(&w_q4k);
    {
        let cmd = queue.new_command_buffer();
        let enc = cmd.new_compute_command_encoder();
        infer_compute::metal::stages::quant_matvec::encode(
            enc, infer_compute::QuantFormat::Q4_K, &w_q4k_buf,
    ...
    let w_q6k = infer_compute::cpu::ops::q4_common::quantize_q6_k(&w_f32);
    let w_q6k_buf = bufs.get_bytes(&w_q6k);
    {
        let cmd = queue.new_command_buffer();
        let enc = cmd.new_compute_command_encoder();
        infer_compute::metal::stages::quant_matvec::encode(
            enc, infer_compute::QuantFormat::Q6_K, &w_q6k_buf,
            &x_buf, 0, &x_buf, 0, &x_buf, 0,
            &out, 0, &pipes, rows, hidden,
        );
        enc.end_encoding();
    ...
    let w_q4_0 = infer_compute::cpu::q4::quantize_q4_0(&w_f32);
    let w_q4_0_buf = bufs.get_bytes(&w_q4_0);
    let (q8_x, q8_x_scales) = infer_compute::cpu::q4::quantize_to_q8(&x);
    ...
    let q8_x_s_buf = bufs.transient_from_f32(&q8_x_scales);
    {
        ...
        let enc = cmd.new_compute_command_encoder();
        infer_compute::metal::stages::quant_matvec::encode(
            enc, infer_compute::QuantFormat::Q4_0, &w_q4_0_buf,
...
fn f16_gemv_matches_f32_gemv_argmax() {
    use infer_models::quant::half::encode_f16;
...
fn q4k_qkv_proj_matches_per_proj_dispatch() {
    ...
    let wq_q4k = infer_compute::cpu::ops::q4_common::quantize_q4_k(wq_f32.as_slice().unwrap());
    let wk_q4k = infer_compute::cpu::ops::q4_common::quantize_q4_k(wk_f32.as_slice().unwrap());
    let wv_q4k = infer_compute::cpu::ops::q4_common::quantize_q4_k(wv_f32.as_slice().unwrap());
    ...
    use infer_compute::metal::shaders::q4k_qkv_proj as sh;
    ...
    let got_q = infer_compute::metal::buffers::read_buffer_f32(&q_out, q_rows);
    let got_k = infer_compute::metal::buffers::read_buffer_f32(&k_out, kv_rows);
    let got_v = infer_compute::metal::buffers::read_buffer_f32(&v_out, kv_rows);
...
fn q4k_q6k_qkv_proj_matches_per_proj_dispatch() {
    ...
    let wq_q4k = infer_compute::cpu::ops::q4_common::quantize_q4_k(wq_f32.as_slice().unwrap());
    let wk_q4k = infer_compute::cpu::ops::q4_common::quantize_q4_k(wk_f32.as_slice().unwrap());
    let wv_q6k = infer_compute::cpu::ops::q4_common::quantize_q6_k(wv_f32.as_slice().unwrap());
    ...
    use infer_compute::metal::shaders::q4k_q6k_qkv_proj as sh;
    let total_rows = (q_rows + kv_rows + kv_rows) as u64;
    let num_tgs = total_rows.div_ceil(sh::ROWS_PER_TG);
    let q_u = q_rows as u32;
    let k_u = kv_rows as u32;
