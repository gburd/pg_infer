...
//! Usage: cargo run --release --features metal -p infer-compute --example bench_decode_q4k
...
fn main() {
    ...
    #[cfg(feature = "metal")]
    {
        ...
        use infer_compute::cpu::ops::q4_common::{quantize_q4_k, quantize_q4_0, quantize_to_q8};
        ...
        // ── Q8 decode_token ──
        let q8_layers: Vec<infer_compute::FullPipelineLayer> = layers_data.iter().map(|ld| {
            infer_compute::FullPipelineLayer {
                wq: infer_compute::QuantWeight { data: &ld.wq_q8, scales: Some(&ld.wq_q8s), format: infer_compute::QuantFormat::Q8_0 },
                wk: infer_compute::QuantWeight { data: &ld.wk_q8, scales: Some(&ld.wk_q8s), format: infer_compute::QuantFormat::Q8_0 },
                wv: infer_compute::QuantWeight { data: &ld.wv_q8, scales: Some(&ld.wv_q8s), format: infer_compute::QuantFormat::Q8_0 },
                wo: infer_compute::QuantWeight { data: &ld.wo_q8, scales: Some(&ld.wo_q8s), format: infer_compute::QuantFormat::Q8_0 },
                gate: infer_compute::QuantWeight { data: &ld.gate_q4, scales: None, format: infer_compute::QuantFormat::Q4_0 },
                up: infer_compute::QuantWeight { data: &ld.up_q4, scales: None, format: infer_compute::QuantFormat::Q4_0 },
                down: infer_compute::QuantWeight { data: &ld.down_q4, scales: None, format: infer_compute::QuantFormat::Q4_0 },
                input_norm: &ld.norm, post_attn_norm: &ld.norm,
