...
//!
//! Usage:
//!   cargo run --release -p infer-compute --features metal --example bench_generation

extern crate blas_src;

use std::time::Instant;
use ndarray::Array2;
use infer_compute::cpu_backend;
...
fn main() {
    ...
    println!("  infer target with Metal + KV:     ~25ms/token → ~40 tok/s");
    println!("  infer current (f32, no KV):      ~220ms/token → ~4.5 tok/s");
