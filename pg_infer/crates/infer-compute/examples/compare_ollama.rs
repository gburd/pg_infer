//! Head-to-head: infer vs Ollama tok/s on the same machine, same moment.
//!
//! Runs infer decode (Q4_K, Q8, raw kernel) then queries Ollama's API,
//! prints a single comparison table. This is THE benchmark to run.
//!
//! Usage: cargo run --release --features metal -p infer-compute --example compare_ollama
//!
...
fn main() {
    ...
    #[cfg(feature = "metal")]
    {
        ...
        use infer_compute::cpu::ops::q4_common::{quantize_q4_k, quantize_to_q8};

        let metal_raw = infer_compute::metal::MetalBackend::new().expect("Metal required");
        ...
        println!("╔═══════════════════════════════════════════════════╗");
        println!("║         infer vs Ollama — Head to Head            ║");
        println!("╚═══════════════════════════════════════════════════╝");
        println!();
        ...
        // ── infer Q4_K decode (21 layers) ──
        let data_21 = build_layers(21);
        let q4k_21: Vec<infer_compute::FullPipelineLayer> = data_21.iter().map(|l| infer_compute::FullPipelineLayer {
            wq: infer_compute::QuantWeight { data: &l.wq, scales: None, format: infer_compute::QuantFormat::Q4_K },
            wk: infer_compute::QuantWeight { data: &l.wk, scales: None, format: infer_compute::QuantFormat::Q4_K },
            wv: infer_compute::QuantWeight { data: &l.wv, scales: None, format: infer_compute::QuantFormat::Q4_K },
            wo: infer_compute::QuantWeight { data: &l.wo, scales: None, format: infer_compute::QuantFormat::Q4_K },
            gate: infer_compute::QuantWeight { data: &l.g, scales: None, format: infer_compute::QuantFormat::Q4_KF },
            up: infer_compute::QuantWeight { data: &l.u, scales: None, format: infer_compute::QuantFormat::Q4_KF },
            down: infer_compute::QuantWeight { data: &l.d, scales: None, format: infer_compute::QuantFormat::Q4_KF },
        ...
        // ── infer Q8 decode (21 layers) ──
        let q8_21: Vec<infer_compute::FullPipelineLayer> = data_21.iter().map(|l| infer_compute::FullPipelineLayer {
            wq: infer_compute::QuantWeight { data: &l.wq8, scales: Some(&l.wq8s), format: infer_compute::QuantFormat::Q8_0 },
            wk: infer_compute::QuantWeight { data: &l.wk8, scales: Some(&l.wk8s), format: infer_compute::QuantFormat::Q8_0 },
            wv: infer_compute::QuantWeight { data: &l.wv8, scales: Some(&l.wv8s), format: infer_compute::QuantFormat::Q8_0 },
            wo: infer_compute::QuantWeight { data: &l.wo8, scales: Some(&l.wo8s), format: infer_compute::QuantFormat::Q8_0 },
            gate: infer_compute::QuantWeight { data: &l.g, scales: None, format: infer_compute::QuantFormat::Q4_KF },
            up: infer_compute::QuantWeight { data: &l.u, scales: None, format: infer_compute::QuantFormat::Q4_KF },
            down: infer_compute::QuantWeight { data: &l.d, scales: None, format: infer_compute::QuantFormat::Q4_KF },
            input_norm: &l.norm, post_attn_norm: &l.norm,
        ...
        // ── infer Q4_K decode (34 layers) ──
        let data_34 = build_layers(34);
        let q4k_34: Vec<infer_compute::FullPipelineLayer> = data_34.iter().map(|l| infer_compute::FullPipelineLayer {
            wq: infer_compute::QuantWeight { data: &l.wq, scales: None, format: infer_compute::QuantFormat::Q4_K },
            wk: infer_compute::QuantWeight { data: &l.wk, scales: None, format: infer_compute::QuantFormat::Q4_K },
            wv: infer_compute::QuantWeight { data: &l.wv, scales: None, format: infer_compute::QuantFormat::Q4_K },
            wo: infer_compute::QuantWeight { data: &l.wo, scales: None, format: infer_compute::QuantFormat::Q4_K },
        ...
        // ── infer raw QKV kernel (34 layers, zero overhead) ──
        ...
        {
            use infer_compute::metal::shaders::q4kf_ffn_gate_up as q4kf_gu;
        ...
        // ── Analysis ──
        println!();
        let per_layer_infer = q4k_21_ms / 21.0;
        ...
        println!("    infer decode:      {per_layer_infer:.3}ms/layer (QKV + attend + FFN + norms)");
        println!("    Ollama decode:     {per_layer_ollama:.3}ms/layer (entire layer)");
        println!("    infer raw kernel:  {per_layer_raw:.3}ms/layer (QKV only, zero overhead)");
        println!();
        ...
        let projected_cached = 1000.0 / (per_layer_infer * 8.0);
