...
`ModelArchitecture` is the core abstraction in `infer-models`. It has 82 methods that describe *what a model is* — tensor key patterns, norm behavior, activation functions, scaling — without any compute dependencies.

Every model family (Gemma, Llama, DeepSeek, ...) implements this trait. The rest of infer (inference, compute, vindex) only interacts with models through this trait.
