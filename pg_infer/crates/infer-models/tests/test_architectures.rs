...
use infer_models::{detect_from_json, ExpertFormat, ModelArchitecture};
...
fn drop_ffn_weights_removes_ffn_tensors() {
    use infer_models::{ModelWeights, WeightArray};
...
fn gemma4_gemma_family_traits() {
    let arch = gemma4_e2b_arch();
    assert_eq!(arch.activation(), infer_models::Activation::GeluTanh);
...
fn mistral_detection_and_keys() {
    ...
    assert_eq!(arch.norm_type(), infer_models::NormType::RmsNorm);
    assert_eq!(arch.activation(), infer_models::Activation::Silu);
    assert_eq!(arch.ffn_type(), infer_models::FfnType::Gated);
}
...
fn starcoder2_norm_and_activation() {
    let arch = starcoder2_arch();
    assert_eq!(arch.norm_type(), infer_models::NormType::LayerNorm);
    assert_eq!(arch.activation(), infer_models::Activation::GeluTanh);
    assert_eq!(arch.ffn_type(), infer_models::FfnType::Standard);
}
